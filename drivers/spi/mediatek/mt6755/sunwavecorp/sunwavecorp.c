...
#define SUNWAVE_VERSION "v0.71.20160903"
...
static int sunwave_read_id_rom(sunwave_sensor_t* sunwave_dev)
{
    ...
    do {
        ...
        sw_info("***read sunwave_fp id rom is %2x-%2x-%2x ***", reg, reg1, reg2);
        err_cnt++;

        if (reg == 0x82 || reg1 == 0x82 || reg2 == 0x82) {
            kfree(sunwave_dev->buffer);
            sunwave_dev->buffer = NULL;
...
static int sunwave_read_id(sunwave_sensor_t* sunwave_dev)
{
    ...
    if (sunwave_read_id_8221(sunwave_dev) == 0) {
		...
		char __user *buffer, size_t count, loff_t *offset)
{
	...
	len += sprintf(data + len, "IC:sunwave.\n");
...
static void create_fp_proc_entry(void)
{
	...
	if (fp_proc_entry) {
		printk(KERN_INFO "create sunwave proc file sucess!\n");
	} else 
		printk(KERN_INFO "create sunwave proc file failed!\n");
}
...
static int __devinit sunwave_dev_probe(struct spi_device* spi)
{
	...
	if(status){
		sw_err("Failed to create chipid file.");
	}
...
struct spi_device_id sunwave_id_table = {FINGERPRINT_DEV_NAME, 0};
...
static int __init sunwave_dev_init(void)
{
	...
	if (sunwave_detected != 1){
		spi_unregister_driver(&sunwave_dev_spi_driver);
		sw_err("failed detect sensor id, unregister spi driver");
	}
    return status;
}
