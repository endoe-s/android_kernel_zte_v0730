...
int mas_sync(u8 *txb, u8 *rxb, int len) {
	int ret;
	// before reading the spi, you must do this , if the mode has changed, delete the next two line
...
int mas_connect(void) {
	...
	for(i=0; i<3; i++){
		...
		if(ret!=0) printw("%s: do mas_sync error! ret = %d\n", __func__, ret);
...
/*	case IOCTL_SET_X:
		...
		break;
	case IOCTL_SET_Y:
		if((x>=0?x:-x)>=(arg>=0?arg:-arg)){
			if(x>=0){
				input_report_key(smas->input, KEY_F6, 1);
				input_sync(smas->input);
				input_report_key(smas->input, KEY_F6, 0);
				input_sync(smas->input);
			}else {
				input_report_key(smas->input, KEY_F7, 1);
				input_sync(smas->input);
				input_report_key(smas->input, KEY_F7, 0);
				input_sync(smas->input);
			}
		}else {
			if(arg>=0){
				input_report_key(smas->input, KEY_F8, 1);
				input_sync(smas->input);
				input_report_key(smas->input, KEY_F8, 0);
				input_sync(smas->input);
			}else{
				input_report_key(smas->input, KEY_F9, 1);
				input_sync(smas->input);
				input_report_key(smas->input, KEY_F9, 0);
				input_sync(smas->input);
			}
		}
		break;
	case IOCTL_KEY_TAP:
			input_report_key(smas->input, KEY_F2, 1);
			input_sync(smas->input);
			input_report_key(smas->input, KEY_F2, 0);
			input_sync(smas->input);		
			break;
	case IOCTL_KEY_DTAP:
			input_report_key(smas->input, KEY_F3, 1);
			input_sync(smas->input);
			input_report_key(smas->input, KEY_F3, 0);
			input_sync(smas->input);
			break;
	case IOCTL_KEY_LTAP:
			input_report_key(smas->input, KEY_F4, 1);
			input_sync(smas->input);
			input_report_key(smas->input, KEY_F4, 0);
			input_sync(smas->input);
			break;
*/	
	case IOCTL_KEY_TAP_ZERO:
			input_report_key(smas->input, KEY_F2, 0);
			input_sync(smas->input);
			//printk(KERN_EMERG "%s: KEY_TAP_ZERO\n", __func__);
			break;
	case IOCTL_KEY_TAP_ONE:
			input_report_key(smas->input, KEY_F2, 1);
			input_sync(smas->input);
			//printk(KERN_EMERG "%s: KEY_TAP_ONE\n", __func__);
			break;
	case IOCTL_KEY_DOWN:
			input_report_key(smas->input, KEY_F3, 1);
			input_sync(smas->input);
	...
	case IOCTL_KEY_UP:
			...
			input_report_key(smas->input, KEY_F4, 0);
			input_sync(smas->input);
			//printk(KERN_EMERG "%s: KEY_UP\n", __func__);
			break;
	case IOCTL_KEY_LEFT:
			input_report_key(smas->input, KEY_F5, 1);
			input_sync(smas->input);
			input_report_key(smas->input, KEY_F5, 0);
			input_sync(smas->input);
			//printk(KERN_EMERG "%s: KEY_LEFT\n", __func__);
			break;
	case IOCTL_KEY_RIGHT:
			input_report_key(smas->input, KEY_F6, 1);
			input_sync(smas->input);
			input_report_key(smas->input, KEY_F6, 0);
			input_sync(smas->input);
			//printk(KERN_EMERG "%s: KEY_RIGHT\n", __func__);
			break;
	}
...
static void mas_set_input(void) {
	...
	set_bit(KEY_F6, input->keybit);
	set_bit(KEY_F7, input->keybit);
	set_bit(KEY_F8, input->keybit);
	set_bit(KEY_F9, input->keybit);
		...
		char __user *buffer, size_t count, loff_t *offset)
{
	...
	len += sprintf(data + len, "IC:microarray afs121n.\n");
...
static void create_fp_proc_entry(void)
{
	...
	if (fp_proc_entry) {
		printk(KERN_INFO "create madev proc file sucess!\n");
	} else 
		printk(KERN_INFO "create madev proc file failed!\n");
}

static ssize_t mas_chipid_show(struct device *dev,   
            struct device_attribute *attr, char *buf)
{   
    return sprintf(buf, "%s\n", "microarray"); 
}
static DEVICE_ATTR(chipid, S_IRUGO, mas_chipid_show, NULL);

static struct attribute *mas_debug_attrs[] = {
    &dev_attr_chipid.attr,
    NULL 
};

static const struct attribute_group mas_debug_attr_group = {
    .attrs = mas_debug_attrs,
    .name = "debug"
};
...
int mas_probe(struct spi_device *spi) {
	...
	smas->spi->controller_data = (void*)&smt_conf;
	spi_setup(spi);
	spin_lock_init(&smas->spi_lock);
	INIT_LIST_HEAD(&smas->dev_entry);
	mutex_lock(&dev_lock);
	ret = mas_connect();
	printw("%s: mas_connect ret=%d\n", __func__, ret);
	if( !ret ) {
...
*/
	...
	ret = sysfs_create_group(&spi->dev.kobj, &mas_debug_attr_group);
	if(ret) {
		printw("%s: create sysfs file failed.\n", __func__);
	}
...
int mas_remove(struct spi_device *spi) {
	...
	sysfs_remove_group(&spi->dev.kobj, &mas_debug_attr_group);
...
static int __init mas_init(void) {
	...
	smas->f_wake = FALSE;
	smas->do_what = 0;
	wake_lock_init(&smas->wl,WAKE_LOCK_SUSPEND,"microarray_wakelock");
	//初始化cdev
	sdev->chd = cdev_alloc();
	...
	} else if (!afs121n_detected){
		printw("%s: spi probe failed.\n", __func__);
		goto fail_plat_probe;
	}
	printd("%s: end\n", __func__);
