...
{		
	...
	fp.ps_rst_disable = pinctrl_lookup_state(fp.pc, "fp_state_rst_input_disable");
	fp.ps_irq_pulldown = pinctrl_lookup_state(fp.pc, "fp_state_eint_pull_down");
	...
	if (IS_ERR(fp.ps_pwr_on) || IS_ERR(fp.ps_pwr_off) || IS_ERR(fp.ps_rst_low) 
	    || IS_ERR(fp.ps_rst_high) || IS_ERR(fp.ps_rst_disable) || IS_ERR(fp.ps_irq_pulldown)		 
		|| IS_ERR(fp.ps_irq_en) || IS_ERR(fp.ps_id_up) || IS_ERR(fp.ps_id_down)) {		
		pr_err("fp:lookup pinctrl state failed");		
		return -1;	
	}
	fp.ps_cs_low = pinctrl_lookup_state(fp.pc, "spi_cs_low");	
	fp.ps_cs_high = pinctrl_lookup_state(fp.pc, "spi_cs_high");	
	fp.ps_mi_low = pinctrl_lookup_state(fp.pc, "spi_mi_low");	
	fp.ps_mi_high = pinctrl_lookup_state(fp.pc, "spi_mi_high");	
	fp.ps_mo_low = pinctrl_lookup_state(fp.pc, "spi_mo_low");	
	fp.ps_mo_high = pinctrl_lookup_state(fp.pc, "spi_mo_high");	
	fp.ps_ck_low = pinctrl_lookup_state(fp.pc, "spi_mclk_low");	
	fp.ps_ck_high = pinctrl_lookup_state(fp.pc, "spi_mclk_high");
	if (IS_ERR(fp.ps_cs_low) || IS_ERR(fp.ps_cs_high) || IS_ERR(fp.ps_mi_low) 
	    || IS_ERR(fp.ps_mi_high) || IS_ERR(fp.ps_mo_low) || IS_ERR(fp.ps_mo_high)		 
		|| IS_ERR(fp.ps_ck_low) || IS_ERR(fp.ps_ck_high) ){		
		pr_err("fp:lookup spi pinctrl state failed");		
		//return -1;	
	}
	...
	if(node) {
		...
		printk(KERN_INFO "fp: irq gpio = %d, irq number %d", fp.irq_gpio, fp.irq_num);		
		if (!fp.irq_num) {
