...
inline static void gf_spi_pins_config(void)
{
	...
	mt_set_gpio_dir(GF_CS_PIN, GPIO_DIR_OUT);
//	mt_set_gpio_pull_enable(GF_CS_PIN, GPIO_PULL_ENABLE);
//	mt_set_gpio_pull_select(GF_CS_PIN, GPIO_PULL_UP);

	/*sck*/
    mt_set_gpio_mode(GF_SCK_PIN, GF_SCK_PIN_M_SCK);
	mt_set_gpio_dir(GF_SCK_PIN, GPIO_DIR_OUT);
//	mt_set_gpio_pull_enable(GF_SCK_PIN, GPIO_PULL_ENABLE);
//	mt_set_gpio_pull_select(GF_SCK_PIN, GPIO_PULL_UP);

	/*miso*/
    mt_set_gpio_mode(GF_MISO_PIN, GF_MISO_PIN_M_MISO);
	mt_set_gpio_dir(GF_MISO_PIN, GPIO_DIR_IN);
//	mt_set_gpio_pull_enable(GF_MISO_PIN, GPIO_PULL_ENABLE);
//	mt_set_gpio_pull_select(GF_MISO_PIN, GPIO_PULL_UP);

	/*mosi*/
    mt_set_gpio_mode(GF_MOSI_PIN, GF_MOSI_PIN_M_MOSI);
	mt_set_gpio_dir(GF_MOSI_PIN, GPIO_DIR_OUT);
//	mt_set_gpio_pull_enable(GF_MOSI_PIN, GPIO_PULL_ENABLE);
//	mt_set_gpio_pull_select(GF_MOSI_PIN, GPIO_PULL_UP);
	...
	spi_pins_default = pinctrl_lookup_state(pinctrl1, "fp_default");
...
//	}
	...
	printk("gf3208:fingerprint_irq=%d\n   ",fingerprint_irq);	
	...
	printk("gf3208:fingerprint_irq ret=%d\n   ",ret);
...
static int __init gf_init(void)
{
	...
	if (!gf3208_detected) {
		...
		gf_error("Failed to probe SPI driver.");
	} else if (status < 0) {
