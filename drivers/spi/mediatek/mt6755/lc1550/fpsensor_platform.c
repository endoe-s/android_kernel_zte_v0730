...
#else
struct mt_chip_conf fpsensor_spi_conf_mt65xx = {    //fpc1020
    .setuptime = 20,
    .holdtime = 20,
    .high_time = 50,
    .low_time = 50,
...
struct mt_chip_conf fpsensor_smt_conf = {
    .setuptime = 1,
    .holdtime = 1,
...
static void spidev_gpio_as_int(fpsensor_data_t *fpsensor)
{
    ...
    pin_select(irq_pulldown);
    mutex_unlock(&spidev_set_gpio_mutex);
}

int fpsensor_plat_power(int power) {
	//power on
	if (power) {
		fpsensor_gpio_output_dts(FPSENSOR_PWR_ON, 1);
	}
	// power off
	else {
		fpsensor_gpio_output_dts(FPSENSOR_PWR_OFF, 1);
	}
...
{
    mutex_lock(&spidev_set_gpio_mutex);
    fpsensor_printk("[fpsensor]fpsensor_gpio_output_dts: gpio= %d, level = %d\n",gpio,level);
	...
	else if (gpio == FPSENSOR_PWR_ON)
	{
		pin_select(pwr_on);
	}
	else if (gpio == FPSENSOR_PWR_OFF)
	{
		pin_select(pwr_off);
	}
	else
	{
		fpsensor_error("fpsensor gpio %d config error, can not find gpio dts info\n", gpio);
	}
    mutex_unlock(&spidev_set_gpio_mutex);
}

int fpsensor_spidev_dts_init(fpsensor_data_t *fpsensor/*, struct platform_device *pdev*/)
{
	/*int ret;

        fpsensor->eint_as_int = pinctrl_lookup_state(fpsensor->pinctrl1, "eint_in_low"); //eint_in_low; eint
...
{
    fpsensor_trace( "[fpsensor]----%s---\n", __func__);
    pdata->reset_gpio = FPSENSOR_RST_PIN;     //-EINVAL;//GPIOM(0);//GPIOC(2);//-EINVAL;
    pdata->irq_gpio   = -EINVAL;              //-EINVAL;
    pdata->cs_gpio    = FPSENSOR_SPI_CS_PIN;  //-EINVAL;//GPIOC(1);//-EINVAL;
    pdata->force_hwid = 0;
...
{
    //if (gpio == this_fpsensor->irq_gpio)
    //    return 1;
...
int fpsensor_irq_init(fpsensor_data_t *fpsensor, struct fpsensor_platform_data *pdata)
{
   ...
   /* node = of_find_compatible_node(NULL, NULL, "mediatek,leadcore-fp");
	if (!node) {
		fpsensor_error("can not find node leadcore-fp, use goodix dts\n");
    ...
    */    fpsensor->irq_gpio = get_gpio(gpio);
        //fpsensor->irq = irq_of_parse_and_map(node, 0);  // get irq number
        fpsensor->irq = get_gpio(num);
        if (!fpsensor->irq)
        {
   ...
   /* }
    ...
    error = request_any_context_irq(fpsensor->irq, fpsensor_interrupt,
                        ...
                        fpsensor_reg_access_t *reg_data)
{
   ...
   // if (fpsensor_gpio_valid(fpsensor->cs_gpio))
    ...
    fpsensor_trace(
        ...
        (reg_data->reg_size > 1) ? ((reg_data->write) ? tx_buffer[cmd_len + 1] : rx_buffer[cmd_len + 1]) : 0,
        (reg_data->reg_size > 2) ? ((reg_data->write) ? tx_buffer[cmd_len + 2] : rx_buffer[cmd_len + 2]) : 0,
        (reg_data->reg_size > 3) ? ((reg_data->write) ? tx_buffer[cmd_len + 3] : rx_buffer[cmd_len + 3]) : 0,
        (reg_data->reg_size > 4) ? ((reg_data->write) ? tx_buffer[cmd_len + 4] : rx_buffer[cmd_len + 4]) : 0,
        (reg_data->reg_size > 5) ? ((reg_data->write) ? tx_buffer[cmd_len + 5] : rx_buffer[cmd_len + 5]) : 0,
        (reg_data->reg_size > 6) ? ((reg_data->write) ? tx_buffer[cmd_len + 6] : rx_buffer[cmd_len + 6]) : 0,
        (reg_data->reg_size > 7) ? ((reg_data->write) ? tx_buffer[cmd_len + 7] : rx_buffer[cmd_len + 7]) : 0);
                         ...
                         size_t buff_size)
{
    int error = 0;
    struct spi_message msg;
    u8* tx_data = buffer + offset;
    u8* rx_data = buffer + offset + 16;

    tx_data[0] = FPSENSOR_CMD_READ_IMAGE;

    fpsensor_trace( "%s (+%d)\n", __func__, offset);

    if ((offset + (int)image_size_bytes + 16) > buff_size) {
        fpsensor_error(
    ...
    if (!error) {
        ...
        //if (fpsensor_gpio_valid(fpsensor->cs_gpio))
            fpsensor_gpio_wirte(fpsensor->cs_gpio, 0);
#endif
        fpsensor_spi_set_mode(fpsensor->spi, (u32)fpsensor->spi_freq_khz, 1);
    ...
    memcpy(buffer + offset, rx_data + 2, image_size_bytes);
