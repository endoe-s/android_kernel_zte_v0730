...
int fpsensor_check_hw_id(fpsensor_data_t *fpsensor)
{
    ...
    if (match) {
        ...
        fpsensor_error( "version_id 0x%x\n", version_id);

        fpsensor_error( "Hardware id: 0x%x (%s, rev.%d) \n",
                         hardware_id,
...
/* -------------------------------------------------------------------- */
int fpsensor_set_esd_val(fpsensor_data_t *fpsensor)
{
    int error = 0;
    u32 temp_u32 = 0;
    fpsensor_reg_access_t reg;
    fpsensor_trace("%s \n", __func__);
    FPSENSOR_MK_REG_READ(reg, FPSENSOR_REG_RDT, &temp_u32);
    error = fpsensor_reg_access(fpsensor, &reg);
    if (error)
    {
        goto out;
    }
    temp_u32 |= FPSENSOR_ESD;
    FPSENSOR_MK_REG_WRITE(reg, FPSENSOR_REG_RDT, &temp_u32);
    error = fpsensor_reg_access(fpsensor, &reg);
    if (error)
    {
        goto out;
...
/* -------------------------------------------------------------------- */
int fpsensor_check_esd_val(fpsensor_data_t *fpsensor)
{
    int error = 0;
    u32 temp_u32 = 0;
    fpsensor_reg_access_t reg;
    ...
    if (FPSENSOR_ESD == (temp_u32 & FPSENSOR_ESD))
    {
        FPSENSOR_MK_REG_WRITE(reg, FPSENSOR_REG_RDT, &temp_u32);
        error = fpsensor_reg_access(fpsensor, &reg);
    }
...
/*----------------------------------------------------------------------*/
int fpsensor_get_finger_DEC_VAL(fpsensor_data_t *fpsensor)
{
    int error = 0 ;
    int i = 0 ;
    u8 buffer_u8[12];
    fpsensor_reg_access_t reg;
    FPSENSOR_MK_REG_READ(reg, FPSENSOR_REG_FNGR_DET_VAL, buffer_u8);
    error = fpsensor_reg_access(fpsensor, &reg);
    fpsensor_trace(" %s [rickon]-------\n", __func__);
    for ( i = 0; i < 12; ++i)
    {
...
int fpsensor_wait_finger_present(fpsensor_data_t *fpsensor)
{
    ...
    fpsensor_trace( "%s\n", __func__);
    fpsensor_get_finger_DEC_VAL(fpsensor);
    error = fpsensor_read_irq(fpsensor, true);
    if (error < 0)
        return error;
    /*
        ...
        while (1) {
            ...
            if (error >= 0) {
                ...
                if (error &
                    (FPSENSOR_IRQ_REG_BIT_FINGER_DOWN |
...
//    sleep_ok = true;
    ...
    while (1) {
       ...
       /* if (0 > fpsensor_check_esd_val(fpsensor))
        {
            ...
            if (error < 0)
                return error;
            fpsensor_capture_finger_detect_settings(fpsensor);
            if (error < 0)
                return error;
            return 113;  //retry
        }*/
        ...
        int measured_val)
{
    ...
    new_val = measured_val + 0x50; // Todo: awaiting calculated values
