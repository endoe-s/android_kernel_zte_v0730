...
static int fpsensor_detected = 0;
...
static int __init fpsensor_init(void)
{
    ...
    }else if (!fpsensor_detected) {
		spi_unregister_driver(&fpsensor_driver);
		fpsensor_error("Failed hardware id not match.\n");
	}

    fpsensor_error("fpsensor_init end\n");
    return status;
}
		...
		char __user *buffer, size_t count, loff_t *offset)
{
	...
	len += sprintf(data + len, "IC:leadcore lc1550.\n");
...
static int fpsensor_probe(struct spi_device *spi)
{
    struct fpsensor_platform_data *fpsensor_pdata = NULL;
    struct fpsensor_platform_data pdata_of = {0};
    ...
    fpsensor_printk("%s, 2016092x -- 3,0x%p\n", __func__, fpsensor);
    ...
    if (!fpsensor_pdata) {
        ...
        if (error) {
			fpsensor_error("fpsensor get pdata failed\n");
            goto err_free_huge_buf;
		}
    }

    error = fpsensor_spidev_dts_init(fpsensor);
	if (error) {
		error = -1;
		fpsensor_error("%s dts init failed!\n", __func__);
		goto err_free_huge_buf;
	}

    fpsensor_param_init(fpsensor, fpsensor_pdata);
	fpsensor_supply_init(fpsensor);
    ...
    if (error)
        goto err_free_irq;
	// here we check if there is a leadcore fingerprint sensor
	...
	error = sysfs_create_group(&spi->dev.kobj,&fp_debug_attr_group);
	if(error){
		fpsensor_error("Failed to create chipid file.");
...
err:
	...
	return error; // FIXME
// err free spi, do we need clean the spi slave info?
err_free_irq:
...
err_free_fpsensor:
	fpsensor_plat_power(0);
	fpsensor_data_uninit();
	kfree(fpsensor);
    return error;
}
...
static int fpsensor_remove(struct spi_device *spi)
{
    ...
    printk("%s\n", __func__);
	sysfs_remove_group(&spi->dev.kobj,&fp_debug_attr_group);
    fpsensor_manage_sysfs(fpsensor, spi, false);
...
//        return -ERESTARTSYS;
    ...
    case FPSENSOR_IOCTL_SET_MUX:
        ...
        if (fpsensor_check_in_range_u64((u8)user_regval, 0, (FPSENSOR_BUFFER_MAX_IMAGES)))
        {
            ...
            fpsensor_trace( "%s, mux = %d, capture_settings_mux = 0x%x\n", __func__, mux, fpsensor->setup.capture_settings_mux);
        }
        else
        {
            error = -EFAULT;
    ...
    case FPSENSOR_IOCTL_SET_CAPTURE_COUNT:
        ...
        if (fpsensor_check_in_range_u64((u8)user_regval, 1, FPSENSOR_BUFFER_MAX_IMAGES))
        {
            ...
            fpsensor_trace( "%s, mux = %d, capture_count = 0x%x\n", __func__, mux, fpsensor->setup.capture_count);
        }
        else
        {
            error = -EFAULT;
    ...
    case FPSENSOR_IOCTL_SET_ROW_START:
        ...
        if (fpsensor_check_in_range_u64((u8)user_regval, 0, (fpsensor->chip.pixel_rows - 1)))
        {
            ...
            fpsensor_trace( "%s, mux = %d, capture_row_start = 0x%x\n", __func__, mux, fpsensor->setup.capture_row_start);
        }
        else
        {
            error = -EFAULT;
    ...
    case FPSENSOR_IOCTL_SET_ROW_COUNT:
        ...
        if (fpsensor_check_in_range_u64((u8)user_regval, 1, fpsensor->chip.pixel_rows))
        {
            ...
            fpsensor_trace( "%s, mux = %d, capture_row_count = 0x%x\n", __func__, mux, fpsensor->setup.capture_row_count);
        }
        else
        {
            error = -EFAULT;
    ...
    case FPSENSOR_IOCTL_SET_COL_START:
        ...
        if (fpsensor_check_in_range_u64((u8)user_regval, 0, (column_groups - 1)))
        {
            ...
            fpsensor_trace( "%s, mux = %d, capture_col_start = 0x%x\n", __func__, mux, fpsensor->setup.capture_col_start);
        }
        else
        {
            error = -EFAULT;
    ...
    case FPSENSOR_IOCTL_SET_COL_GROUPS:
        ...
        if (fpsensor_check_in_range_u64((u8)user_regval, 1, column_groups))
        {
            ...
            fpsensor_trace( "%s, mux = %d, capture_col_groups = 0x%x\n", __func__, mux, fpsensor->setup.capture_col_groups);
        }
        else
        {
            error = -EFAULT;
    ...
    case FPSENSOR_IOCTL_WRITE_TINY_CAPTURE:
        ...
        fpsensor_trace( "%s, mux = %d, tiny_capture_enable = 0x%x\n", __func__, mux, fpsensor->setup.tiny_capture_enable);
        break;
    ...
    case FPSENSOR_IOCTL_SET_UP_THRESHOLD:
        ...
        fpsensor_trace( "%s, mux = %d, capture_finger_up_threshold = 0x%x\n", __func__, mux, fpsensor->setup.capture_finger_up_threshold);
        break;
    ...
    case FPSENSOR_IOCTL_SET_DOWN_THRESHOLD:
        ...
        fpsensor_trace( "%s, mux = %d, capture_finger_down_threshold = 0x%x\n", __func__, mux, fpsensor->setup.capture_finger_down_threshold);
        break;
    case FPSENSOR_IOCTL_SET_DETECT_THRESHOLD:
        ...
        fpsensor_trace( "%s, mux = %d, finger_detect_threshold = 0x%x\n", __func__, mux, fpsensor->setup.finger_detect_threshold);
        break;
...
//    fpsensor_trace( "%s\n", __func__);
    ...
    error = fpsensor_cmd(fpsensor, FPSENSOR_CMD_CAPTURE_IMAGE, false);//FPSENSOR_IRQ_REG_BIT_FIFO_NEW_DATA
