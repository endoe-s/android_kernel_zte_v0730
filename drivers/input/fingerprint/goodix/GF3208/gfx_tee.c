...
#define GF_NAV_TOUCH_KEY    KEY_MENU 
#else
#define GF_NAV_UP_KEY       195 
#define GF_NAV_DOWN_KEY     196 
#define GF_NAV_LEFT_KEY     197 
#define GF_NAV_RIGHT_KEY    198 
#define GF_NAV_TOUCH_KEY    199 
#endif
...
/* for netlink use */
static int pid = 0;
struct gf_device *gf_dev_p = NULL;
...
void gf_power_on(bool onoff)
{
	gf_debug(INFO_LOG,"power=%d\n",onoff);
	if(onoff)
	{
		mt_set_gpio_mode(GPIO_FP_INT_PIN, GPIO_MODE_00);
		...
		mt_set_gpio_mode(GFX1XM_LDO2_8V_EN_PIN, GFX1XM_LDO_EN_PIN_M_GPIO);
		mt_set_gpio_dir(GFX1XM_LDO2_8V_EN_PIN, GPIO_DIR_OUT);
		mt_set_gpio_out(GFX1XM_LDO2_8V_EN_PIN, GPIO_OUT_ONE);				
		mdelay(20);
	...
	else
	{
		...
		mt_set_gpio_mode(GFX1XM_LDO2_8V_EN_PIN, GFX1XM_LDO_EN_PIN_M_GPIO);
		mt_set_gpio_dir(GFX1XM_LDO2_8V_EN_PIN, GPIO_DIR_OUT);
		mt_set_gpio_out(GFX1XM_LDO2_8V_EN_PIN, GPIO_OUT_ZERO);
		mdelay(40);
	}
}

inline static void gf_gpio_cfg(void)
{
	...
	secspi_enable_clk(gf_dev_p->spi);
	
	gf_debug(INFO_LOG, "gf516m_gpio_cfg success\n");
	return;
}
...
static void gf_spi_clk_enable(struct gf_device *gf_dev, u8 bonoff)
{
	...
	if (bonoff && (count == 0)) {
		...
		secspi_enable_clk(gf_dev_p->spi);// FOR MT6755/MT6750
		count = 1;
	} else if ((count > 0) && (bonoff == 0)) {
		//mt_spi_disable_clk(ms);    // FOR MT6797
		secspi_disable_clk(gf_dev_p->spi); // FOR MT6755/MT6750
		count = 0;
	}
#endif
}
...
{	
	#if 1
	gf_irq_cfg();
	gf_dev->irq_num=CUST_EINT_FP_EINT_NUM;
	gf_dev->irq = mt_gpio_to_irq(CUST_EINT_FP_EINT_NUM);
	gf_debug(INFO_LOG,"gf_irq_gpio_cfg irq_num(%d) irq(%d)\n", gf_dev->irq_num,gf_dev->irq);
	#else
	...
	{
		gf_dev->irq = gf_dev->irq_num=CUST_EINT_FP_EINT_NUM;
		gf_debug(INFO_LOG," can't find compatible node,CUST_EINT_FP_EINT_NUM(%d)\n", CUST_EINT_FP_EINT_NUM);
	}
#endif
	#endif
}
...
#ifdef SUPPORT_REE_SPI
static void gf_set_cmd(struct gf_device *gf_dev, u8 cmd)
{
	...
	*/
	...
	case GF_IOC_INPUT_KEY_EVENT:
		...
		if (GF_KEY_UP == gf_key.key) {
			...
			input_report_key(gf_dev->input, GF_NAV_TOUCH_KEY, 1);
			input_sync(gf_dev->input);
			input_report_key(gf_dev->input, GF_NAV_TOUCH_KEY, 0);
			input_sync(gf_dev->input);
		}
		#if 0
			...
			struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (!strncmp(buf, "-8", 2)) {
		...
		secspi_disable_clk(gf_dev_p->spi);
...
int gf_spi_write_bytes_ree(struct gf_device *gf_dev, u16 addr, u32 data_len, u8 *tx_buf)
{
	...
	package = (data_len + 5) / 1024;
	reminder = (data_len + 5) % 1024;
	...
	*(tmp_buf + 1) = (u8)((addr >> 8) & 0xFF);
	*(tmp_buf + 2) = (u8)(addr & 0xFF);
	*(tmp_buf + 3) = (u8)((data_len/2 >> 8) & 0xFF);
	*(tmp_buf + 4) = (u8)(data_len/2 & 0xFF);
...
static int gf_check_9p_chip(struct gf_device *gf_dev)
{
	...
	do {
		...
		gf_spi_read_bytes_ree(gf_dev, 0x0000, 2, tmp_buf);
		gf_spi_read_bytes_ree(gf_dev, 0x0002, 2, tmp_buf+2);
		gf_debug(INFO_LOG, "%s, chip version is 0x%x, 0x%x, 0x%x, 0x%x\n", __func__,
				tmp_buf[0], tmp_buf[1], tmp_buf[2], tmp_buf[3]);

		time_out++;
		/* 9P MP chip version is 0x00900802*/
		if ((0x02 == tmp_buf[0]) && (0xA0 == tmp_buf[1]) && (0x00 == tmp_buf[2])&& (0x22 == tmp_buf[3])) {
...
static int gf_probe(struct spi_device *spi)
{
	...
	__set_bit(GF_NAV_TOUCH_KEY, gf_dev->input->keybit);
