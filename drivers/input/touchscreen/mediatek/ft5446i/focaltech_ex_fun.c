...
int tpd_get_chipvendor_from_bootloader(struct i2c_client *i2c_client, int *vendorid_in_chip, int * chip_part_id)
{
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) {
		...
		if ((reg_val[0] == fts_updateinfo_curr.upgrade_id_1
			&& reg_val[1] == fts_updateinfo_curr.upgrade_id_2) || g_force_upgrade) {
			DBG("tpd [FTS] Step 3: READ OK CTPM ID,ID1 = 0x%x,ID2 = 0x%x\n",
				reg_val[0], reg_val[1]);
			break;
		} else {
	...
	*vendorid_in_chip = auc_i2c_write_buf[4];
	...
	int *fwver_in_chip, int *vendorid_in_chip)
{
	...
	while (i < 2) {
		i++;
		fts_read_reg(i2c_client, FTS_REG_CHIP_ID, &uc_reg_value);
		*chipid_in_chip = uc_reg_value;
		fts_read_reg(i2c_client, FTS_REG_FW_VER, &uc_reg_value);
		*fwver_in_chip = uc_reg_value;
		fts_read_reg(i2c_client, FTS_REG_VENDOR_ID, &uc_reg_value);
		*vendorid_in_chip = uc_reg_value;
		
		if(0x00 == *vendorid_in_chip || 0xff == *vendorid_in_chip || FTS_REG_VENDOR_ID == *vendorid_in_chip) {
...
int tpd_fts_ctpm_firmware_upgrade(struct i2c_client *client, unsigned char * pbt_buf, unsigned int size, unsigned int force_upg)
{
	int i_ret = 0;;

	g_force_upgrade = force_upg;
...
struct i2c_client * g_focal_i2c_client;
/*interface of write proc*/
static ssize_t fts_debug_write(struct file *filp,
					 const char *buff, size_t len,
					 loff_t * off)
{
	struct i2c_client *client = g_focal_i2c_client;//(struct i2c_client *)fts_proc_entry->data;
...
static ssize_t fts_debug_read(struct file *file,
	char __user *buff, size_t len, loff_t *data)
{
	struct i2c_client *client = g_focal_i2c_client;//(struct i2c_client *)fts_proc_entry->data;
	int ret = 0;
