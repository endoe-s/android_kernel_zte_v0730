...
static int tpd_fb_notifier_callback(struct notifier_block *self, unsigned long event, void *data)
{
	...
	TPD_DEBUG("tpd_fb_notifier_callback0707\n");

	evdata = data;
	
	blank = *(int *)evdata->data;
	TPD_DMESG("fb_notify(event=%ld) (blank=%d)\n", event, blank);
	
	/* If we aren't interested in this event, skip it immediately ... */
	if (event != FB_EARLY_EVENT_BLANK && event != FB_R_EARLY_EVENT_BLANK && event != FB_EVENT_BLANK)
		return 0;
	...
	if( (FB_BLANK_UNBLANK == blank && FB_EVENT_BLANK == event) || 
		(FB_BLANK_POWERDOWN == blank && FB_R_EARLY_EVENT_BLANK == event)) {
		TPD_DMESG("LCD ON Notify\n");
		if (g_tpd_drv && tpd_suspend_flag) {
			err = queue_work(touch_resume_workqueue, &touch_resume_work);
			if (!err) {
	...
	}else if( FB_BLANK_POWERDOWN == blank && FB_EARLY_EVENT_BLANK == event) {
		TPD_DMESG("LCD OFF Notify\n");
		if (g_tpd_drv) {
			err = cancel_work_sync(&touch_resume_work);
			if (!err)
		...
		TPD_DMESG("not handle this event && blank.\n");
	}
