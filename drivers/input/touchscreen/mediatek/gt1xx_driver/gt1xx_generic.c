...
{
	gt1x_debug_proc_entry = proc_create(GT1X_DEBUG_PROC_FILE, 0644, NULL, &gt1x_debug_fops);
...
s32 gt1x_init_panel(void)
{
...
#if GTP_DRIVER_SEND_CFG
	u8 sensor_id = 0;
	struct goodix_ts_data *goodix_data = i2c_get_clientdata(gt1x_i2c_client);
	...
	//for tianma sanxing com
	const u8 cfg_grp6_tianma[] = GTP_CFG_GROUP0_TIANMA;
	const u8 cfg_grp7_sanxing[] = GTP_CFG_GROUP0_SANXING;
	const u8 *cfgs[] = {
		cfg_grp0, cfg_grp1, cfg_grp2,
		cfg_grp3, cfg_grp4, cfg_grp5, 
		cfg_grp6_tianma, cfg_grp7_sanxing,
	};
	u8 cfg_lens[] = {
		...
		CFG_GROUP_LEN(cfg_grp6_tianma),
		CFG_GROUP_LEN(cfg_grp7_sanxing),
	};
	...
	GTP_INFO("check lcm name = %s", tmp_name);
	if(0 == strcmp(tmp_name, "hx8392a_dsi_cmd")){
		GTP_INFO("lcm = hx8392a_dsi_cmd tianma.");
		is_lcd_tianma = 1;
	}
	if(0 == strcmp(tmp_name, "sumsing_s6e8_hd_vdo_dsi")){
		GTP_INFO("lcm = sumsing_s6e8_hd_vdo_dsi sanxing.");
		is_lcd_sanxing = 1;
	}

	GTP_DEBUG("Config groups length:%d,%d,%d,%d,%d,%d, %d, %d", cfg_lens[0], cfg_lens[1], cfg_lens[2], cfg_lens[3], 
		cfg_lens[4], cfg_lens[5], cfg_lens[6], cfg_lens[7]);
	...
	goodix_data->sensor_id = sensor_id;
...
s32 gt1x_read_version(struct gt1x_version_info * ver_info)
{
	...
	if (buf[3] == 0x00) {    //get tp chip number 1151 970 etc.
		goodix_data->chip_part_id = ascii2hex(buf[0]) * 100 + ascii2hex(buf[1]) * 10 + ascii2hex(buf[2]);
	} else {
		goodix_data->chip_part_id = ascii2hex(buf[0]) * 1000 + ascii2hex(buf[1]) * 100 + ascii2hex(buf[2]) * 10 + ascii2hex(buf[3]);
	}
	//goodix_data->sensor_id = sensor_id;
	goodix_data->firmware_version = patch_id;
	return 0;
}
...
{
	if(0 != gt1x_version.sensor_id) {
		GTP_INFO("not boen module return.");
		return;
	}
	...
	char __user *buffer, size_t count, loff_t *offset)
{
	...
	struct goodix_ts_data *ts_data = i2c_get_clientdata(gt1x_i2c_client);
	...
	len += sprintf(data + len, "Goodix Touchscreen 20161223.\n");
	len += sprintf(data + len, "tpd is suspend:%d , need stay awake:%d , bsg enable? :%d , sensor id:%d .\n", ts_data->tpd_suspend, ts_data->need_stay_awake, 
		ts_data->enable_wakeup_gesture, gt1x_version.sensor_id);
...
static int tpd_init_tpinfo(struct tpd_classdev_t *cdev)
{
	...
	if(9157 == data->chip_part_id ||970 ==data->chip_part_id ||615 == data->chip_part_id 
		||915 == data->chip_part_id || 1151 == data->chip_part_id ||5668 == data->chip_part_id){
		;
	}else{
	...
	cdev->ic_tpinfo.module_id = data->sensor_id;
	if(0x0 == data->sensor_id) {
		cdev->ic_tpinfo.module_id = 0x12;
	} else  if(0x1 == data->sensor_id) {
		cdev->ic_tpinfo.module_id = 0x05;
	}
	cdev->ic_tpinfo.config_ver = data->config_version;
	cdev->ic_tpinfo.firmware_ver= data->firmware_version;
...
extern int tpd_gt1x_update_start(struct tpd_classdev_t *cdev, unsigned char * data, unsigned int size);
extern int tpd_gt1x_compare_firmware_version(struct tpd_classdev_t *cdev, unsigned char * data, unsigned int size, int force_upg);

/*retrun value 0: need upgrade    0xff: not need update*/
static int tpd_compare_tp(struct tpd_classdev_t *cdev, unsigned char *data)
{
	return tpd_gt1x_compare_firmware_version(cdev, data, cdev->tp_fw.data_len, 0);
}
...
static int tpd_flash_firmware(struct tpd_classdev_t *cdev, unsigned char * data, unsigned int size, int force_upg)
{
	...
	i_ret = tpd_gt1x_compare_firmware_version(cdev, data, cdev->tp_fw.data_len, force_upg);
	if( i_ret != 0)	{
		GTP_INFO("%s line:%d RMI check firmware finished, return...\n", __func__, __LINE__);
		goto out;
	}

	i_ret = tpd_gt1x_update_start(cdev, data, size);	
	if (i_ret == 0) {
...
int tpd_register_fw_class(struct goodix_ts_data *data)
{	
	//tpd_fw_cdev.name = "touchscreen";//for tp load after selinux init.
	...
	//tpd_classdev_register(&(data->i2c_client->dev), &tpd_fw_cdev);//for tp load after selinux init.
...
s32 gt1x_init(void)
{
	...
	//while (retry++ < 1) {
		...
		if (ret != 0) {
			GTP_ERROR("gt1x_i2c_read_dbl_check not pass[0x%2X].", reg_val[0]);
			//continue;
			//gt1x_init_failed = 1;
			//goto i2c_test_error;
