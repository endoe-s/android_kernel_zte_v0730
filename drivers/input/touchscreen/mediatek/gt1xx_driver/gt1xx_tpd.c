...
static struct debug_point_desc global_debug_point_desc[DEBUG_POINT_NUM];
...
static void debug_point(int id, int x, int y, int w, int down)
{
	...
	if(id < DEBUG_POINT_NUM && 1 == down) {
		pre_finger_status = global_debug_point_desc[id].finger_status;
		global_var_point_num++;
		global_debug_point_desc[id].finger_status = down;
		global_debug_point_desc[id].downnum++;
		global_debug_point_desc[id].x = x;
		global_debug_point_desc[id].y = y;
		down_num = global_debug_point_desc[id].downnum;
		if((0 == pre_finger_status && 1 == down) || down_num % 90 == 0) {
			//printk("tpd down\n");
			//printk("tpd d %d(%d,%d)%d D%d Q%ld N%ld\n", id, x, y, w, 
			//	global_debug_point_desc[id].downnum, global_var_irq_num, global_var_point_num);
			printk("tpd D %d(%d,%d)%d D%d\n", id, x, y, w, global_debug_point_desc[id].downnum);
		}
	} else if(0 == down) {
		for(i = 0; i < DEBUG_POINT_NUM; i++) {
			if(1==global_debug_point_desc[i].finger_status) {
				//printk("tpd up\n");
				printk("tpd u %d(%d,%d) D%d\n",i, global_debug_point_desc[i].x, global_debug_point_desc[i].y, global_debug_point_desc[i].downnum);
			}
			global_debug_point_desc[i].finger_status=0;
			global_debug_point_desc[i].downnum=0;
			global_debug_point_desc[i].x=0;
			global_debug_point_desc[i].y=0;
		}
	}
}
...
static s32 tpd_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	s32 idx = 0;
#endif
	struct goodix_ts_data *goodix_data = NULL;

	GTP_INFO("Enter %s, %d\n", __FUNCTION__, __LINE__);

	if (RECOVERY_BOOT == get_boot_mode()) {   // important
		GTP_INFO("RECOVERY_BOOT return. %s, %d\n", __FUNCTION__, __LINE__);
		return 0;
	}

#ifdef GTP_CONFIG_OF
	tpd->reg = regulator_get(tpd->tpd_dev, "vtouch");
	err = regulator_set_voltage(tpd->reg, 3000000, 3000000);
	if (err != 0) {
	...
	if (gt1x_init() != 0) {
		...
		gt1x_abs_x_max = 0;
		gt1x_abs_y_max = 0;
		GTP_ERROR(TPD_DEVICE " gtp_init_error.n");
		goto gtp_init_error;
	}
...
//static void gt1x_change_point(int *x, int *y)
//{
//	*x = TPD_RES_X - (*x);
//	*y = TPD_RES_Y - (*y) ;
//}
