...
#ifdef CONFIG_CREATE_SYS_INTERFACE
int tpd_gt1x_update_prepare(unsigned char * data, unsigned int size)
{
	int ret = 0;
	int retry = 5;
	...
	update_info.fw_name = NULL;
	update_info.update_type = UPDATE_TYPE_HEADER;
	update_info.fw_data = data;
	update_info.fw_length = size;
...
void tpd_gt1x_update_cleanup(void)
{
	if (update_info.buffer != NULL) {
		kfree(update_info.buffer);
		update_info.buffer = NULL;
...
int tpd_gt1x_check_firmware(void)
{
	u16 checksum;
	u16 checksum_in_header;
	u8 *p;
...
int tpd_gt1x_update_judge(int i)
{
	int ret;
	u8 reg_val[2] = {0};
	u8 retry = 2;
...
#endif
	if ((fw_ver_info.patch_id & 0xFFFF) <= (ver_info.patch_id & 0xFFFF) && !update_info.force_update) {
		GTP_INFO("The version of the fw is not high than the IC's!");
		return ERROR_CHECK;
	}
	return 0;
...
int tpd_gt1x_compare_firmware_version(struct tpd_classdev_t *cdev, unsigned char * data, unsigned int size, int force_upg)
{
	s32 retval = 0;
	//s32 i = 0;
	//s32 fw_checksum = 0;

	GTP_INFO("%s func in, force upgrade %d .\n", __func__, force_upg);

	update_info.force_update = force_upg;

	retval = tpd_gt1x_update_prepare(data, size);
	if (retval) {
		update_info.status = UPDATE_STATUS_IDLE;
		retval = 0xff;
		goto out_0;
	...
	retval = tpd_gt1x_check_firmware();
	if (retval) {
		update_info.status = UPDATE_STATUS_IDLE;
		retval = 0xff;
		goto out_1;
	...
	{
		u8 * buf = &update_info.firmware->pid[0];
		int chip_id = 0;
		if (buf[3] == 0x00) {    //get tp chip number 1151 970 etc.
			chip_id = ascii2hex(buf[0]) * 100 + ascii2hex(buf[1]) * 10 + ascii2hex(buf[2]);
		} else {
			chip_id = ascii2hex(buf[0]) * 1000 + ascii2hex(buf[1]) * 100 + ascii2hex(buf[2]) * 10 + ascii2hex(buf[3]);
		}
		cdev->file_tpinfo.chip_part_id = chip_id;
	}
	cdev->file_tpinfo.firmware_ver = (update_info.firmware->version[1] << 8) | update_info.firmware->version[2];
	
	retval = tpd_gt1x_update_judge(1);
	if(retval) {
		GTP_INFO("%s not upgrade retval:%d .\n", __func__, retval);
		retval = 0xff;
	} else {
		GTP_INFO("%s upgrade retval:%d .\n", __func__, retval);
		retval = 0;
	}
	
out_1:
	tpd_gt1x_update_cleanup();
out_0:
	update_info.force_update = 0;
	update_info.status = UPDATE_STATUS_IDLE;
	
	return retval;
}

int tpd_gt1x_update_firmware(unsigned char * data, unsigned int size)
{
	int i = 0;
	int ret = 0;
	u8 *p;

	GTP_INFO("%s func in, size %d .\n", __func__, size);
	...
	ret = tpd_gt1x_update_prepare(data, size);
	if (ret) {
		update_info.status = UPDATE_STATUS_IDLE;
		return ret;
	}

	ret = tpd_gt1x_check_firmware();
	if (ret) {
		update_info.status = UPDATE_STATUS_ABORT;
		goto gt1x_update_exit;
	}
...
int tpd_gt1x_update_start(struct tpd_classdev_t *cdev, unsigned char * data, unsigned int size)
{
	int retval = -1;

	GTP_INFO("%s func in, size %d .\n", __func__, size);

	update_info.force_update = 1;
	
	retval = tpd_gt1x_update_firmware(data, size);
	if(retval == 0) {
		GTP_INFO("%s , upgrade success, retval %d .\n", __func__, retval);
		retval = 0;
	} else {
		GTP_INFO("%s , upgrade failed, retval %d .\n", __func__, retval);
		retval = -1;
	}
