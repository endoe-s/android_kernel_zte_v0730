...
void tpd_process_blackwakeup_func(struct goodix_ts_data *data, u8 gesture, struct input_dev * dev)
{
	unsigned int code = KEY_RESERVED;

	//a,b,c,d,e,g,h,m,o,q,s,v,w,y,z,^
	if ((gesture == 'c') || (gesture == 'e') || (gesture == 'm') || 
		(gesture == 's') || (gesture == 'w') || (gesture == 'o') ) {
		...
		GTP_INFO("%c gesture light up the screen!", gesture);
		...
		GTP_INFO("double click gesture light up the screen!");
	} else {
		GTP_INFO("undefined gesture light up the screen!");
	}
...
s32 gesture_event_handler(struct input_dev * dev)
{
	...
	GTP_INFO("tpd 0x%x = 0x%02X,0x%02X,0x%02X,0x%02X", GTP_REG_WAKEUP_GESTURE,
	...
	if (extra_len >= 0) {
		...
		if (likely(need_chk)) { /* calc checksum */
			...
			if (unlikely(!val)) { /* check failed */
				GTP_ERROR("Gesture extra checksum error.");
				if (err_flag1) {
					err_flag1 = 0;
					ret = 0;
					goto clear_reg;
	...
	if (len > 0) { /* coor num * 4 + chksum 2*/
		u8 ges_data[len * 4 + 2];
		u16 ges_buffer_addr = 0;

		ges_buffer_addr = 0x8A40; //GES_BUFFER_ADDR; for gt5668 8a40;
	...
	tpd_process_blackwakeup_func(NULL, doze_buf[0], dev);
