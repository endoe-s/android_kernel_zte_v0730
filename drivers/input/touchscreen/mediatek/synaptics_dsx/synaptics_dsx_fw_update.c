...
int synaptics_fw_updater_by_tpd(unsigned char *fw_data, int force)
{
	int retval;
...
int synaptics_get_f34_crtl_info(struct synaptics_rmi4_data *ts, unsigned int * chip_id, unsigned int * vendor_id, unsigned int * fw_version)
{
	...
	chipid_in_chip = buf[0];
	vendorid_in_chip = buf[1];
	fwver_in_chip = (buf[2] << 8) | (buf[3] & 0xff);
	printk("tpd f34 type:0x%x, Partner:0x%x, Fw_rev:0x%x, in chip\n", 
		chipid_in_chip, vendorid_in_chip, fwver_in_chip);
	if(chipid_in_chip == 0x00 || vendorid_in_chip == 0x00 || 
		chipid_in_chip == 0xff || vendorid_in_chip == 0xff || fwver_in_chip == 0xffff) {
		printk("%s may be upgrade failed.\n", __func__);
		fwver_in_chip = 0x00;
	}
	synaptics_rmi4_reg_read(ts, ts->f01_query_base_addr, product_id, 4);
	chipid_in_chip = 0x49;    //product_id[3] + (product_id[3] >= 0x29 ? 0x18 : 0x12);
	vendorid_in_chip = 0x35;    //product_id[2] + (product_id[2] >= 0x29 ? 0x18 : 0x12);
	
	*chip_id = chipid_in_chip;
	*vendor_id = vendorid_in_chip;
	*fw_version = fwver_in_chip;

	return 0;
}

int synaptics_check_fw_version(struct tpd_classdev_t *cdev, unsigned char * data, unsigned int size, unsigned int force_upg)
{
	unsigned int chipid_in_chip, vendorid_in_chip, fwver_in_chip;
	...
	fwu->image = data;
	fwu_parse_image_info();
	//fwu_scan_pdt();

	synaptics_get_f34_crtl_info(ts, &chipid_in_chip, &vendorid_in_chip, &fwver_in_chip);
		
	chipid_in_file = fwu->img.ui_config.data[0];
	vendorid_in_file = fwu->img.ui_config.data[1];
	fwver_in_file=((fwu->img.ui_config.data[2]) << 8) | ((fwu->img.ui_config.data[3]) & 0xff);

	printk("tpd Type:0x%x, Partner:0x%x, Fw_rev:0x%x, in chip\n", 
		chipid_in_chip, vendorid_in_chip, fwver_in_chip);
	printk("tpd Type:0x%x, Partner:0x%x, Fw_rev:0x%x, in new firmware\n", 
		chipid_in_file, vendorid_in_file, fwver_in_file);
	
	strcpy(cdev->file_tpinfo.tp_name, cdev->ic_tpinfo.tp_name);
	cdev->file_tpinfo.chip_part_id= tpd_get_chip_id(chipid_in_file);
	cdev->file_tpinfo.module_id= vendorid_in_file;
	cdev->file_tpinfo.chip_ver = cdev->ic_tpinfo.chip_ver;
	cdev->file_tpinfo.firmware_ver = fwver_in_file;
	...
	if(force_upg || ((chipid_in_chip == chipid_in_file) && (vendorid_in_chip == vendorid_in_file) && 
		...
		printk("tpd %s: check firmware finish, need upgrade fw!\n", __func__);
		return 0;
	}

	printk("tpd %s: this chip is't need upgrade firmware\n", __func__);
