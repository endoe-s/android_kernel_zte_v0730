...
static int global_var_point_num = 0;
static struct synaptics_rmi4_exp_fn_data exp_data;
static struct synaptics_rmi4_data *g_rmi4_data = NULL;

static struct device_attribute attrs[] = {
#ifdef CONFIG_HAS_EARLYSUSPEND
	__ATTR(full_pm_cycle, (S_IWUSR | S_IRUGO),
	...
	__ATTR(flashprog, S_IRUGO,
			...
			synaptics_rmi4_store_error),
	__ATTR(0dbutton, (S_IWUSR | S_IRUGO),
			synaptics_rmi4_0dbutton_show,
			synaptics_rmi4_0dbutton_store),
	__ATTR(suspend, S_IWUSR,
			synaptics_rmi4_show_error,
			synaptics_rmi4_suspend_store),
	__ATTR(wake_gesture, S_IRUGO,
		...
		unsigned short addr, unsigned char *data, unsigned short length)
{
	...
	for (ii = 1; ii <= total; ii++) {
		...
		read_msg[ii].buf = (unsigned char *)I2CDMABuf_pa + I2C_DMA_LIMIT * (ii - 1);
	...
	struct i2c_msg msg[] = {
		{
			...
			.flags = 0,
			.len = length + 1,
			.buf = (unsigned char *)I2CDMABuf_pa,
			.ext_flag= (rmi4_data->i2c_client->ext_flag | 
...
static void debug_point(int id, int x, int y, int w, int down)
{
	if((global_var_point_num - 1) % 90 == 0) {
		printk("tpd:%d%d %d,%d\n", id, down, x, y);
	}

	//if(ts_debug)
	//	printk("point: id%d sta:%d x=%d, y=%d, w=%d\n", id, down, x, y, w);
}

static int synaptics_handle_wakegesture_event(struct synaptics_rmi4_data *rmi4_data, unsigned char gesture, unsigned char *data)
{
	unsigned int code = KEY_RESERVED;

	printk("%s wakegesture:0x%02x.\n", __func__, gesture);

	//a,b,c,d,e,g,h,m,o,q,s,v,w,y,z,^
	switch(gesture) {
	case 0x08:
		code = KEY_GESTURE_O;
		printk("circle light up the screen\n");
		break;
	case 0x20:
		code = KEY_GESTURE_W;
		printk("V light up the screen\n");
		break;
	case 0x40:
		{
			int i = 0;
...
//			if(tpd_fw_cdev.ic_tpinfo.vendor_id == 0x32 && tpd_fw_cdev.ic_tpinfo.firmware_ver >= 0x3046 ||
//				tpd_fw_cdev.ic_tpinfo.vendor_id == 0x31 && tpd_fw_cdev.ic_tpinfo.firmware_ver >= 0x3144 ||
//				tpd_fw_cdev.ic_tpinfo.vendor_id == 0x41 && tpd_fw_cdev.ic_tpinfo.firmware_ver >= 0x3139)
			acsii_pos = 8;
//#endif
			
			switch(data[acsii_pos]) {
			case 'c':
			case 'C':
				code = KEY_GESTURE_C;
				printk("%c  light up the screen\n", data[acsii_pos]);
				break;
			case 'e':
			case 'E':
				code = KEY_GESTURE_E;
	...
	case 0x02:
		{
			...
			printk("%s:data is 0x%x 0x%x 0x%x 0x%x\n",	__func__, data[0], data[1], data[2], data[3]);
			...
			printk("%s slide to light up the screen!\n", direction[type]);
			if (type == 0){
		...
		break;
	default:
		printk("%s gesture not recongize.\n", __func__);
		break;
	}
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	unsigned char detected_gestures[12];    //unsigned char detected_gestures; //bsg
	...
	if (rmi4_data->sensor_sleep && rmi4_data->enable_wakeup_gesture) {
		retval = synaptics_rmi4_i2c_read(rmi4_data,
				...
				&detected_gestures[0], 12);
				//&detected_gestures,
				//sizeof(detected_gestures));    //bsg
		if (retval < 0)
			return 0;
		...
		//bsg
		synaptics_handle_wakegesture_event(rmi4_data, detected_gestures[0], &detected_gestures[1]);
		...
		bool enable)
{
	...
	spin_lock_irqsave(&rmi4_data->irq_lock, flag);
	if (enable) {
		if (!rmi4_data->irq_enabled) {
#ifdef SYNAPTICS_CONFIG_OF
	...
	spin_unlock_irqrestore(&rmi4_data->irq_lock, flag);
	
	return retval;
}
#ifdef SYNAPTICS_CONFIG_OF
static irqreturn_t tpd_eint_interrupt_handler(unsigned irq, void *data)
{
...
static int synaptics_irq_registration(struct synaptics_rmi4_data *rmi4_data)
{
#ifdef SYNAPTICS_CONFIG_OF
	...
	printk("tpd Device Tree Tpd_irq_registration!");
	...
	if(node){
		...
		rmi4_data->irq = irq_of_parse_and_map(node, 0);
		retval = request_irq(rmi4_data->irq, (irq_handler_t)tpd_eint_interrupt_handler, IRQF_TRIGGER_LOW, 
			"TOUCH_PANEL-eint", (void *) rmi4_data);
		if(retval > 0){
			retval = -1;
			printk("tpd request_irq IRQ LINE NOT AVAILABLE!.");
		}
	}else{
		printk("tpd tpd request_irq can not find touch eint device node!.");
		retval = -1;
	}
	printk("tpd irq:%d, debounce:%d-%d:", rmi4_data->irq, ints[0], ints[1]);
	rmi4_data->irq_enabled = true;
	return retval;
...
static void synaptics_rmi4_set_params(struct synaptics_rmi4_data *rmi4_data)
{
...
#ifdef TYPE_B_PROTOCOL
	input_mt_init_slots(rmi4_data->input_dev, rmi4_data->num_of_fingers, INPUT_MT_DIRECT);
#endif
	...
	input_set_capability(rmi4_data->input_dev, EV_KEY, KEY_POWER);    //FOR wakeup TEST
	input_set_capability(rmi4_data->input_dev, EV_KEY, KEY_GESTURE_DOUBLE_CLICK); 
	input_set_capability(rmi4_data->input_dev, EV_KEY, KEY_GESTURE_UP); 
	input_set_capability(rmi4_data->input_dev, EV_KEY, KEY_GESTURE_DOWN);
	input_set_capability(rmi4_data->input_dev, EV_KEY, KEY_GESTURE_LEFT); 
	input_set_capability(rmi4_data->input_dev, EV_KEY, KEY_GESTURE_RIGHT); 
	...
	input_set_capability(rmi4_data->input_dev, EV_KEY, KEY_GESTURE_O);
	input_set_capability(rmi4_data->input_dev, EV_KEY, KEY_GESTURE_S);
	input_set_capability(rmi4_data->input_dev, EV_KEY, KEY_GESTURE_U); 
	...
	char __user *buffer, size_t count, loff_t *offset)
{
	...
	int finger = 0, ret = 0;
	ssize_t len = 0;
	struct synaptics_rmi4_data *data = g_rmi4_data;
	int i = 0;
	...
	len += sprintf(buffer_synap+len, "Synaptics Touchscreen v20160129.\n");
	...
	//len += sprintf(buffer_synap+len, "irq flag:0x%lx.\n", data->hw_if->board_data->irq_flags);
	...
	len += sprintf(buffer_synap + len, "device control register val \"0x04:nosleep, 0x0:normal, 0x1:sleep. \"  control reg:0x%x , irq status reg: 0x%x doze reg:0x%x.\n",
		irq_status[0], irq_status[1], irq_status[2]);

	len += sprintf(buffer_synap + len, "x,y:(0x%x, 0x%x), rx:%d, tx:%d finger:%d .\n", data->sensor_max_x, data->sensor_max_y, 
		data->num_of_rx, data->num_of_tx, data->num_of_fingers);
	...
	ret = synaptics_rmi4_reg_read(data, 0x57, buf, 16);
	if (ret < 0) {
		len += sprintf(buffer_synap+len, "i2c_transfer failed\n");
	} else {
		len += sprintf(buffer_synap+len, "F11 Ctrl: :"
			"%x %x %x %x %x %x %x %x %x"
				" %x %x %x %x %x %x, ret %d\n",
					   buf[0], buf[1], buf[2], buf[3],
					   buf[4], buf[5], buf[6], buf[7],
	...
	ret = synaptics_rmi4_reg_read(data, 0x00dd, buf, 18);
	if (ret < 0) {
		len += sprintf(buffer_synap+len, "i2c_transfer failed\n");
	} else {
		for(i = 0; i < 3; i++) {
			len += sprintf(buffer_synap+len, "fun:0x%02x, query:0x%02x command:0x%02x, control:0x%02x, data:0x%02x, version:0x%02x.\n",
				buf[i * 6 + 5], buf[i * 6], buf[i * 6 + 1], buf[i * 6 + 2], buf[i * 6 + 3], buf[i * 6 + 4]);
		}
	}
	/*
	...
	len += sprintf(buffer_synap + len, "rst pin 0x%x, mode:%d, dir:%d, pullen:%d, sel:%d, inver:%d, outval:%d, in:%d.\n",
	...
	len += sprintf(buffer_synap + len, "int pin 0x%x, mode:%d, dir:%d, pullen:%d, sel:%d, inver:%d, outval:%d, in:%d.\n",
...
//}
#endif
extern int synaptics_get_f34_crtl_info(struct synaptics_rmi4_data *ts, unsigned int * chip_id, unsigned int * vendor_id, unsigned int * fw_version);
extern int synaptics_check_fw_version(struct tpd_classdev_t *cdev, unsigned char * data, unsigned int size, unsigned int force_upg);
...
{
	unsigned int chipid_in_chip, vendorid_in_chip, fwver_in_chip;
	struct synaptics_rmi4_data *ts = (struct synaptics_rmi4_data*) cdev->private;

	// Read current firmware revision, partner etc...
	synaptics_get_f34_crtl_info(ts, &chipid_in_chip, &vendorid_in_chip, &fwver_in_chip);
	...
	cdev->ic_tpinfo.chip_part_id = tpd_get_chip_id(chipid_in_chip);
	...
	cdev->ic_tpinfo.i2c_addr = ts->i2c_client->addr;
	
	return 0;
}

static int tpd_check_fw_version(struct tpd_classdev_t *cdev, unsigned char * data, unsigned int size, unsigned int force_upg)
{
	return synaptics_check_fw_version(cdev, data, size, force_upg);
}
...
static int tpd_flash_firmware(struct tpd_classdev_t *cdev, unsigned char * data, unsigned int size, int force_upg)
{
	...
	synaptics_data->stay_awake = true;
	if(synaptics_data->touch_stopped) {
	...
	i_ret = tpd_check_fw_version(cdev, data, cdev->tp_fw.data_len, force_upg);
	if( i_ret != 0)	{
	...
	if (i_ret == 0) {
		...
		i_ret = synaptics_fw_updater_by_tpd(data, 1);
		if( 0 != i_ret )	{
			TPD_DMESG("upgrade failed ret=%d.\n", i_ret);	
		} else {
			TPD_DMESG("upgrade firmware success at second try.\n");
		}
	}
	tpd_init_tpinfo(cdev);
	
	synaptics_data->stay_awake = false;
out:
	return i_ret;
}
...
static int tpd_compare_tp(struct tpd_classdev_t *cdev, unsigned char *data)
{
	...
	i_ret = tpd_check_fw_version(cdev, data, cdev->tp_fw.data_len, 0);
...
static void synaptics_rmi4_gpio_reset(void)
{
	printk("%s func in.\n", __func__);
#ifdef SYNAPTICS_CONFIG_OF
...
static int synaptics_rmi4_power_on(struct synaptics_rmi4_data *rmi4_data, int on)
{
	int retval = -1;
	...
	case 1:
		if (power_state == 0) {    
			printk("tpd Power switch on!");
			if(!tpd_power_already_on) {
#ifdef SYNAPTICS_CONFIG_OF
				//enable regulator
				retval = regulator_enable(tpd->reg);
				if (retval != 0) {
					printk("tpd regulator_enable() failed!");
					return retval;
				}
				synaptics_rmi4_gpio_reset();
#else
				// gpio setting
				mt_set_gpio_mode(GPIO_CTP_EINT_PIN, GPIO_CTP_EINT_PIN_M_EINT);
				mt_set_gpio_dir(GPIO_CTP_EINT_PIN, GPIO_DIR_IN);
	...
	case 0:
		if (power_state == 1) {
			printk("tpd Power switch off!");
#ifdef SYNAPTICS_CONFIG_OF
			GTP_GPIO_OUTPUT(GTP_RST_PORT, 0);
			//disable regulator
			retval = regulator_disable(tpd->reg);
			if (retval != 0) {
				printk("tpd regulator_disable() failed!");
			}
#else
		...
		break;
	default:
		printk("tpd Invalid power switch command!");
		break;
	}
	return 0;
		...
		const struct i2c_device_id *dev_id)
{
	...
	I2CDMABuf_va = (u8 *)dma_alloc_coherent(&client->dev, 256, &I2CDMABuf_pa, GFP_KERNEL);
	if(!I2CDMABuf_va)
	{
		dev_err(&client->dev,
			"%s: Allocate Touch DMA I2C Buffer failed!\n",__func__);
		return -ENOMEM;
	}
	...
	if (retval != 0) {
		dev_err(&client->dev, "%s: Failed to set reg-vgp1 voltage: %d\n", __func__, retval);
		return -1;
	}
#endif
	...
	retval = synaptics_rmi4_power_on(rmi4_data, 1);
	...
	retval = synaptics_irq_registration(rmi4_data);
...
#ifdef SYNAPTICS_CONFIG_OF
#define SYNAPTICS_DRIVER_NAME  "synaptics_touch"
static const struct i2c_device_id goodix_i2c_id[] = { {SYNAPTICS_DRIVER_NAME, 0}, {} };
...
static struct i2c_driver tpd_i2c_driver = {
	...
	.driver.name = SYNAPTICS_DRIVER_NAME,
	.driver = {
...
{
	//printk("synaptics I2C Touchscreen Driver (Built %s @ %s)\n", __DATE__, __TIME__);
	printk("synaptics I2C Touchscreen Driver %s.\n", __func__);

	if(i2c_add_driver(&tpd_i2c_driver)!=0)
	{
		printk("Error: unable to add i2c driver.\n");
		return -1;
	}
#ifdef TPD_HAVE_BUTTON     
