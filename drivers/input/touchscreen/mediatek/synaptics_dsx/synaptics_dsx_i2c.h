...
#define SYNAPTICS_MTK_DRIVER_VERSION 0x6020
...
static inline int tpd_get_chip_id(int id)
{
	int i = 0;

	for(i = 0; i < sizeof(synap_chip_id_l); i ++) {
		if(id == synap_chip_id_l[i].id) {
			return synap_chip_id_l[i].chip_model;
		} else if(0xff == synap_chip_id_l[i].id) {
			return id;
		}
	}
	return 0;
