...
#endif
struct wake_lock zte_onekey_lock;//add by tony 
...
static void kpd_keymap_handler(unsigned long data)
{
	...
	for (i = 0; i < KPD_NUM_MEMS; i++) {
		...
		for (j = 0; j < 16; j++) {
			...
			if(linux_keycode == 212)
				wake_lock_timeout(&zte_onekey_lock, 3*HZ);
		}
	}
...
static int kpd_pdrv_probe(struct platform_device *pdev)
{
	...
	wake_lock_init(&zte_onekey_lock, WAKE_LOCK_SUSPEND, "zte onekey  wakelock");//add by tony 
	mt_eint_register();
