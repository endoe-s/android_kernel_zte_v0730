...
static void step_c_poll(unsigned long data)
{
	...
	if (obj != NULL)
		//zhangxin 20160421 add for freezable workqueue;
		//schedule_work(&obj->report);
		queue_work(obj->stepC_workqueue, &obj->report);
}
...
static struct step_c_context *step_c_context_alloc_object(void)
{
	...
	//zhangxin 20160421 add for freezable workqueue;
	obj->stepC_workqueue = NULL;
	obj->stepC_workqueue = create_freezable_workqueue("stepC_polling");
	if (!obj->stepC_workqueue) {
		STEP_C_ERR("create workqueue error!\n");
		kfree(obj);
		return NULL;
	}
...
int step_c_driver_add(struct step_c_init_info *obj)
{
	...
	STEP_C_FUN();
	for (i = 0; i < MAX_CHOOSE_STEP_C_NUM; i++) {
		//modify by dgsh 201604, add (NULL == step_counter_init_list[0]) for compatible more than one device.
		if ((i == 0) && (NULL == step_counter_init_list[0])) 
		{
