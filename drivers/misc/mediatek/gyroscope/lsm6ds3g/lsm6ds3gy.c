...
#define LSM6DS3_TAG                  "[LSM6DS3-Gyro] "

#define LSM6DS3_FUN(f)               printk( LSM6DS3_TAG"%s\n", __FUNCTION__)
#define LSM6DS3_ERR(fmt, args...)    printk(KERN_ERR LSM6DS3_TAG"%s %d : "fmt, __FUNCTION__, __LINE__, ##args)
#define LSM6DS3_LOG(fmt, args...)    printk( LSM6DS3_TAG fmt, ##args)
...
static void LSM6DS3_power(struct gyro_hw *hw, unsigned int on) 
{
	...
	{        
		...
		{
			LSM6DS3_LOG("ignore power control: %d\n", on);
		}
		else if(on)	// power on
		{
			if(!hwPowerOn(hw->power_id, hw->power_vol, "LSM6DS3"))
			{
				LSM6DS3_ERR("power on fails!!\n");
			}
		}
		else	// power off
		{
...
static int LSM6DS3_gyro_write_rel_calibration(struct lsm6ds3_gyro_i2c_data *obj, int dat[LSM6DS3_GYRO_AXES_NUM])
{
    ...
    obj->cali_sw[LSM6DS3_AXIS_Z] = obj->cvt.sign[LSM6DS3_AXIS_Z]*dat[obj->cvt.map[LSM6DS3_AXIS_Z]];
		...
		{
			LSM6DS3_LOG("test  (%5d, %5d, %5d) ->(%5d, %5d, %5d)->(%5d, %5d, %5d))\n", 
			...
			LSM6DS3_LOG("write gyro calibration data  (%5d, %5d, %5d)\n", 
...
static int LSM6DS3_gyro_ReadCalibration(struct i2c_client *client, int dat[LSM6DS3_GYRO_AXES_NUM])
{
    ...
    dat[obj->cvt.map[LSM6DS3_AXIS_Z]] = obj->cvt.sign[LSM6DS3_AXIS_Z]*obj->cali_sw[LSM6DS3_AXIS_Z];
		...
		if(atomic_read(&obj->trace) & GYRO_TRC_CALI)
		{
			LSM6DS3_LOG("Read gyro calibration data  (%5d, %5d, %5d)\n", 
				dat[LSM6DS3_AXIS_X],dat[LSM6DS3_AXIS_Y],dat[LSM6DS3_AXIS_Z]);
		}
#endif
...
static int LSM6DS3_gyro_WriteCalibration(struct i2c_client *client, int dat[LSM6DS3_GYRO_AXES_NUM])
{
	...
	{        		
		...
		if(atomic_read(&obj->trace) & GYRO_TRC_CALI)
		{
			LSM6DS3_LOG("write gyro calibration data  (%5d, %5d, %5d)-->(%5d, %5d, %5d)\n", 
				dat[LSM6DS3_AXIS_X], dat[LSM6DS3_AXIS_Y], dat[LSM6DS3_AXIS_Z],
...
static int LSM6DS3_gyro_SetPowerMode(struct i2c_client *client, bool enable)
{
	...
	{
		LSM6DS3_LOG("Sensor power status is newest!\n");
		return LSM6DS3_SUCCESS;
	}

	if(hwmsen_read_byte(client, LSM6DS3_CTRL2_G, databuf))
	{
		LSM6DS3_ERR("read lsm6ds3 power ctl register err!\n");
		return LSM6DS3_ERR_I2C;
	}
	...
	if(res <= 0)
	{
		LSM6DS3_LOG("LSM6DS3 set power mode: ODR 100hz failed!\n");
		return LSM6DS3_ERR_I2C;
	}	
	else
	{
		LSM6DS3_LOG("set LSM6DS3 gyro power mode:ODR 100HZ ok %d!\n", enable);
	}
...
static int LSM6DS3_Set_RegInc(struct i2c_client *client, bool inc)
{
	...
	{
		LSM6DS3_ERR("read LSM6DS3_CTRL1_XL err!\n");
		return LSM6DS3_ERR_I2C;
	}
	else
	{
		LSM6DS3_LOG("read  LSM6DS3_CTRL1_XL register: 0x%x\n", databuf[0]);
	}
	if(inc)
	{
		...
		if(res <= 0)
		{
			LSM6DS3_ERR("write full scale register err!\n");
			return LSM6DS3_ERR_I2C;
		}
	}
	return LSM6DS3_SUCCESS;    
...
static int LSM6DS3_gyro_SetFullScale(struct i2c_client *client, u8 gyro_fs)
{
	...
	if(hwmsen_read_byte(client, LSM6DS3_CTRL2_G, databuf))
	{
		LSM6DS3_ERR("read LSM6DS3_CTRL2_G err!\n");
		return LSM6DS3_ERR_I2C;
	}
	else
	{
		LSM6DS3_LOG("read  LSM6DS3_CTRL2_G register: 0x%x\n", databuf[0]);
	}
...
static int LSM6DS3_gyro_SetSampleRate(struct i2c_client *client, u8 sample_rate)
{
	...
	if(hwmsen_read_byte(client, LSM6DS3_CTRL2_G, databuf))
	{
		LSM6DS3_ERR("read gyro data format register err!\n");
		return LSM6DS3_ERR_I2C;
	}
	else
	{
		LSM6DS3_LOG("read  gyro data format register: 0x%x\n", databuf[0]);
	}
	...
	if(res <= 0)
	{
		LSM6DS3_ERR("write sample rate register err!\n");
		return LSM6DS3_ERR_I2C;
	}
...
static int LSM6DS3_ReadGyroData(struct i2c_client *client, char *buf, int bufsize)
{
	...
	{
		LSM6DS3_ERR("LSM6DS3 read gyroscope data  error\n");
		return -2;
	}
	else
	{
		...
		{
			LSM6DS3_LOG("read gyro register: %x, %x, %x, %x, %x, %x",
				databuf[0], databuf[1], databuf[2], databuf[3], databuf[4], databuf[5]);
			LSM6DS3_LOG("get gyro raw data (0x%08X, 0x%08X, 0x%08X) -> (%5d, %5d, %5d)\n", 
				obj->data[LSM6DS3_AXIS_X],obj->data[LSM6DS3_AXIS_Y],obj->data[LSM6DS3_AXIS_Z],
				obj->data[LSM6DS3_AXIS_X],obj->data[LSM6DS3_AXIS_Y],obj->data[LSM6DS3_AXIS_Z]);
			LSM6DS3_LOG("get gyro cali data (%5d, %5d, %5d)\n", 
				obj->cali_sw[LSM6DS3_AXIS_X],obj->cali_sw[LSM6DS3_AXIS_Y],obj->cali_sw[LSM6DS3_AXIS_Z]);
		}
#endif	
#if 1
		obj->data[LSM6DS3_AXIS_X] = (s64)(obj->data[LSM6DS3_AXIS_X]) * LSM6DS3_GYRO_SENSITIVITY_2000DPS*3142/(180*1000*1000);
		obj->data[LSM6DS3_AXIS_Y] = (s64)(obj->data[LSM6DS3_AXIS_Y]) * LSM6DS3_GYRO_SENSITIVITY_2000DPS*3142/(180*1000*1000);
		obj->data[LSM6DS3_AXIS_Z] = (s64)(obj->data[LSM6DS3_AXIS_Z]) * LSM6DS3_GYRO_SENSITIVITY_2000DPS*3142/(180*1000*1000); 
	...
	{
		LSM6DS3_LOG("get gyro data packet:[%d %d %d]\n", data[0], data[1], data[2]);
	}
#endif
...
static ssize_t show_chipinfo_value(struct device_driver *ddri, char *buf)
{
	...
	if(NULL == client)
	{
		LSM6DS3_ERR("i2c client is null!!\n");
		return 0;
	}
...
static ssize_t show_trace_value(struct device_driver *ddri, char *buf)
{
	...
	if (obj == NULL)
	{
		LSM6DS3_ERR("i2c_data obj is null!!\n");
		return 0;
	}
...
static ssize_t store_trace_value(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	{
		LSM6DS3_ERR("invalid content: '%s', length = %zu\n", buf, count);
	}
...
*/
	LSM6DS3_LOG("LSM6DS3_gyro_init_client OK!\n");
	//acc setting
...
static int lsm6ds3_gyro_enable_nodata(int en)
{
	...
	if(priv == NULL)
	{
		LSM6DS3_ERR("obj_i2c_data is NULL!\n");
		return -1;
	}
	...
	LSM6DS3_LOG("enable value=%d, sensor_power =%d\n",value,sensor_power);
	if(((value == 0) && (sensor_power == false)) ||((value == 1) && (sensor_power == true)))
	{
		LSM6DS3_LOG("Gsensor device have updated!\n");
	}
	else
	{	
	...
	LSM6DS3_dumpReg(priv->client);

    LSM6DS3_LOG("mc3xxx_enable_nodata OK!\n");
    return err;
}
...
//#if 0					
	...
	{
		LSM6DS3_ERR("Set delay parameter error!\n");
	}
...
//#endif
    //LSM6DS3_LOG("mc3xxx_set_delay (%d), chip only use 1024HZ \n",value);
    LSM6DS3_LOG("lsm6ds3_GYRO_set_delay (%d), ODR = 0x%x\n",value, sample_delay);
    return 0;
}
	...
	switch (cmd) {
		 case COMPAT_GYROSCOPE_IOCTL_INIT:
			 ...
			 ret = file->f_op->unlocked_ioctl(file, GYROSCOPE_IOCTL_INIT,
							(unsigned long)arg32);
			 if (ret){
			 	LSM6DS3_ERR("GYROSCOPE_IOCTL_INIT unlocked_ioctl failed.");
				return ret;
			 }			 
		 ...
		 case COMPAT_GYROSCOPE_IOCTL_SET_CALI:
			 ...
			 ret = file->f_op->unlocked_ioctl(file, GYROSCOPE_IOCTL_SET_CALI,
							(unsigned long)arg32);
			 if (ret){
			 	LSM6DS3_ERR("GYROSCOPE_IOCTL_SET_CALI unlocked_ioctl failed.");
				return ret;
			 }			 
		 ...
		 case COMPAT_GYROSCOPE_IOCTL_CLR_CALI:
			 ...
			 ret = file->f_op->unlocked_ioctl(file, GYROSCOPE_IOCTL_CLR_CALI,
							(unsigned long)arg32);
			 if (ret){
			 	LSM6DS3_ERR("GYROSCOPE_IOCTL_CLR_CALI unlocked_ioctl failed.");
				return ret;
			 }			 
		 ...
		 case COMPAT_GYROSCOPE_IOCTL_GET_CALI:
			 ...
			 ret = file->f_op->unlocked_ioctl(file, GYROSCOPE_IOCTL_GET_CALI,
							(unsigned long)arg32);
			 if (ret){
			 	LSM6DS3_ERR("GYROSCOPE_IOCTL_GET_CALI unlocked_ioctl failed.");
				return ret;
			 }			 
		 ...
		 case COMPAT_GYROSCOPE_IOCTL_READ_SENSORDATA:
			 ...
			 ret = file->f_op->unlocked_ioctl(file, GYROSCOPE_IOCTL_READ_SENSORDATA,
							(unsigned long)arg32);
			 if (ret){
			 	LSM6DS3_ERR("GYROSCOPE_IOCTL_READ_SENSORDATA unlocked_ioctl failed.");
				return ret;
			 }			 
       ...
       unsigned long arg)
{
	...
	if(err)
	{
		LSM6DS3_ERR("access error: %08X, (%2d, %2d)\n", cmd, _IOC_DIR(cmd), _IOC_SIZE(cmd));
		return -EFAULT;
	}
	...
	switch(cmd)
	{
		...
		case GYROSCOPE_IOCTL_SMT_DATA:
			...
			LSM6DS3_LOG("IOCTL smtRes: %d!\n", smtRes);
			copy_cnt = copy_to_user(data, &smtRes,  sizeof(smtRes));
			
			if(copy_cnt)
			{
				err = -EFAULT;
				LSM6DS3_ERR("copy gyro data to user failed!\n");
			}	
			LSM6DS3_LOG("copy gyro data to user OK: %d!\n", copy_cnt);
			break;
		...
		default:
			LSM6DS3_ERR("unknown IOCTL: 0x%08x\n", cmd);
			err = -ENOIOCTLCMD;
			break;			
	}
	return err;
...
static int lsm6ds3_gyro_suspend(struct i2c_client *client, pm_message_t msg) 
{
	...
	{   
		...
		if(err)
		{
			LSM6DS3_ERR("write power control fail!!\n");
			return err;
		}
...
static int lsm6ds3_gyro_resume(struct i2c_client *client)
{
	...
	if(err)
	{
		LSM6DS3_ERR("initialize client fail! err code %d!\n", err);
		return err;        
	}
...
static int lsm6ds3_gyro_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	LSM6DS3_LOG("lsm6ds3_gyro_i2c_probe-1:malloc obj OK\n");
	...
	if(err)
	{
		LSM6DS3_ERR("invalid direction: %d\n", obj->hw->direction);
		goto exit;
	}
	...
	lsm6ds3_i2c_client->addr = 0x6a;

	LSM6DS3_LOG("lsm6ds3_gyro_i2c_probe-2: connect hw & obj OK\n");
	...
	LSM6DS3_LOG("lsm6ds3_gyro_i2c_probe-3: init_client OK\n");
		
	err = misc_register(&lsm6ds3_gyro_device);
	if(err)
	{
		LSM6DS3_ERR("lsm6ds3_gyro_device misc register failed!\n");
	...
	if(err)
	{
		LSM6DS3_ERR("lsm6ds3 create attribute err = %d\n", err);
		goto exit_create_attr_failed;
	}	

	LSM6DS3_LOG("lsm6ds3_gyro_i2c_probe-4: misc register OK\n");
    ...
    if(err)
    {
         LSM6DS3_ERR("register acc control path err\n");
        goto exit_kfree;
    }
    ...
    if(err)
    {
        LSM6DS3_ERR("register acc data path err= %d\n", err);
        goto exit_kfree;
    }

	LSM6DS3_LOG("lsm6ds3_gyro_i2c_probe-5: register ctl & data path OK\n");
...
static int lsm6ds3_gyro_i2c_remove(struct i2c_client *client)
{
	...
	if(err)
	{
		LSM6DS3_ERR("lsm6ds3_gyro_i2c_remove fail: %d\n", err);
	}
	...
	if(err)
	{
		LSM6DS3_ERR("misc_deregister lsm6ds3_gyro_device fail: %d\n", err);
	}
	#endif
...
static int lsm6ds3_gyro_local_init(struct platform_device *pdev)
{
	...
	{
		LSM6DS3_ERR("add driver error\n");
		return -1;
	}
	if(lsm6ds3_gyro_init_flag == -1)
	{
		LSM6DS3_ERR("%s init failed!\n", __FUNCTION__);
		return -1;
	}
	return 0;
}
...
{
	const char *name = "mediatek,lsm6ds3g";

	LSM6DS3_FUN();

	hw =   get_gyro_dts_func(name, hw);
	if (!hw)	{
		LSM6DS3_ERR("get dts info fail\n");
		return 0;
	}
