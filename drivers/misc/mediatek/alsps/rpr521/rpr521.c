...
static const struct of_device_id alsps_of_match[] = {
	{.compatible = "mediatek,alsps-rpr521"},
	{},
};
#endif
...
static void rpr521_check_prox_mean(unsigned int prox_mean)
{
	...
	if(prox_mean <= 100)
	{
	    THRES_TOLERANCE = 35; // 25;
        THRES_DEFAULT_DIFF = 50; //35;  
	}
...
int rpr521_setup_eint(struct i2c_client *client)
{
	...
	if (request_irq(alsps_irq, rpr521_eint_func, IRQF_TRIGGER_LOW, "ALS-eint", NULL)) {
...
{
	rpr521_proc_file = proc_create("driver/tsl2771_threshold", 0666, NULL, &rpr521_proc_fops);
    APS_FUN(f);
...
//get adc channel 1 value
	...
	APS_DBG("\nROHM rpr521-als read als: als_data0 = %d\n", als_data.als_data0);
	als_data.als_data1 = als_ch1_value_low[0] | (als_ch1_value_high[0]<<8);
	APS_DBG("\nROHM rpr521-als read als: als_data1 = %d\n", als_data.als_data1);
	...
	*data = calc_rohm_als_data(als_data, dev_val);
    ...
    APS_DBG("\nROHM rpr521-als read als output data = %d\n", *data);
...
long rpr521_read_ps(struct i2c_client *client, u16 *data)
{
	...
	*data = (ps_value_low[0] | (ps_value_high[0]<<8))& 0xfff;
	...
	printk("ROHM rpr521--ps_data=%d, low:%d  high:%d", *data, ps_value_low[0], ps_value_high[0]); 
	printk("ROHM rpr521--mode=%x, int_sts:%x", mode[0], int_sts[0]);
...
static int rpr521_get_threshold(struct i2c_client *client)
{
	...
	average = get_median(tmpdata);
   	printk("rpr521_get_threshold the average is :%d!\n", average);
	if(0 == rpr521_ps_original_power)
      {
	     rpr521_enable_ps(rpr521_i2c_client, 0);
	     clear_bit(CMC_BIT_PS, &obj->enable);
	}
	return average;
...
static int rpr521_set_threshold(int average)
{
	...
	printk("rpr521_set_threshold,average=%d\n", average);
	if(average < 0)
	{		
		goto EXIT_ERR;
	}
