...
/* ======================================================================== */
//lihy add start
static unsigned long g_flag1 = 0xFF;
extern void eng_flash_enable(void);
extern void eng_flash_disable(void);

static ssize_t show_flash_status(struct device *dev, struct device_attribute *attr, char *buf)
{
    int ret = 0;
...
static ssize_t write_flash_status(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	int rc = -1;
	
   logI("lihy write buf = %s \n", buf);
   rc = kstrtoul(buf, 10, &g_flag1);
    if (rc)
		return rc;
   logI("lihy g_flag1 = %ld \n", g_flag1);
    if (1 == g_flag1) {
        eng_flash_enable();
    }else if (0 == g_flag1) {
        eng_flash_disable();
...
static DEVICE_ATTR(flash_onoff, 0664, show_flash_status, write_flash_status);

static struct attribute *dev_attrs[] = {
	&dev_attr_flash_onoff.attr,
	NULL,
};
...
static int msm_flash_lm3646_add_attr(struct platform_device *pdev)
{
	int ret = -1;

    if (NULL != pdev )
    {
        logI("lihy pdev->name=%s   %s\n",pdev->name,pdev->dev.kobj.name);
        ret = sysfs_create_group(&pdev->dev.kobj, &dev_attr_grp);
        if (ret)
            logI("%s: Failed to create sysfs node: %d\n", __func__, ret);
    }
...
#define FLASHLIGHT_MAJOR 242
	...
	//flashlight_gpio_init(dev);  lihy 20161018  no need 

	//lihy add start
	ret = msm_flash_lm3646_add_attr(dev);
    //endf
...
static struct platform_driver flashlight_platform_driver = {
	...
	.driver = {
		   ...
		   .owner = THIS_MODULE,
#ifdef CONFIG_OF
	//.of_match_table = FLASHLIGHT_of_match,  lihy 20161018  no need 
#endif
		   },
};
...
static int __init flashlight_init(void)
{
	...
	logI("[flashlight_init] start ~");

	ret = platform_device_register(&flashlight_platform_device);
	if (ret) {
		logI("[flashlight_init] platform_driver_register fail ~");
		return ret;
	}

	ret = platform_driver_register(&flashlight_platform_driver);
	if (ret) {
		logI("[flashlight_init] platform_device_register fail ~");
		return ret;
	}
