	...
	if (pCamCalList != NULL && pCamCalFunc != NULL) {
		...
		for (i = 0; pCamCalList[i].sensorID != 0; i++) {
			...
			if (pCamCalList[i].sensorID == sensorID) {
				...
				CAM_CALDB("g_i2c_info[%d].addr =%x\n", g_curDevIdx,(*g_i2c_info[g_curDevIdx]).addr);
...
static long cam_cal_drv_compat_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    CAM_CALDB("lihy COMPAT_CAM_CALIOC_G_READ = %x", (unsigned int)COMPAT_CAM_CALIOC_G_READ);
	
	switch (cmd) {

	case COMPAT_CAM_CALIOC_G_READ: {
		CAM_CALDB("COMPAT_CAM_CALIOC_G_READ---lihy");
		data32 = compat_ptr(arg);
		data = compat_alloc_user_space(sizeof(*data));
		if (data == NULL)
		{
			CAM_CALDB("COMPAT_CAM_CALIOC_G_READ---lihy data = NULL");
			return -EFAULT;
		}
		...
		if (err)
		{
			CAM_CALDB("cam_cal_compat_get_info---err");
			return err;
		}
	...
	case COMPAT_CAM_CALIOC_S_WRITE: {/*Note: Write Command is Unverified!*/
		CAM_CALDB("COMPAT_CAM_CALIOC_S_WRITE---lihy");
		data32 = compat_ptr(arg);
		data = compat_alloc_user_space(sizeof(*data));
		if (data == NULL)
			return -EFAULT;
...
#endif
	default:
		CAM_CALDB("cam_cal_drv_compat_ioctl default---lihy End!\n");
		return -ENOIOCTLCMD;
	}
