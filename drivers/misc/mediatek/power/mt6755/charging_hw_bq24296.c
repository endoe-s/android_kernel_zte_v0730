...
#include <linux/delay.h>//linuxÎÄ¼þ±»ÆÁ±Î£¿
...
//#include <cust_gpio_usage.h>// no file
...
const kal_uint32 CS_VTH[]=
{
	...
	230400, 236800, 343200, 249600,
	256000, 262400, 268800, 275200,
	281600, 280000, 294400, 300800
}; 
...
static kal_uint32 charging_hw_init(void *data)
{
	...
	#if 0 //#if defined(GPIO_SWCHARGER_EN_PIN)
	...
	bq24296_set_iterm(0x0); //Termination current 128mA
      ...
      #if defined(MAX_CHARGE_VOLTAGE_4400)
	  bq24296_set_vreg(0x38); //VREG 4.4V,0x38
      #else
...
static kal_uint32 charging_set_cv_voltage(void *data)
{
	...
	if(BATTERY_VOLT_04_200000_V == cv_value)
		cv_value = 4208000;
	if(BATTERY_VOLT_04_350000_V == cv_value)
		cv_value = 4352000;
...
static kal_uint32 charging_set_hv_threshold(void *data)
{
	...
	pmic_set_register_value(PMIC_RG_VCDT_LV_VTH,0);//VCDT_LV_VTH  4.2v
	return status;
}
...
//	kal_uint32 status = STATUS_OK;
	...
	retval = request_irq(CUST_EINT_VDC_NUM, IRQF_TRIGGER_LOW,
	                     xhci_eint_iddig_isr,"bq24296_diso_eint" ,NULL);
	if (retval != 0) {
...
unsigned int set_shipping_mode_enable(void *data)
{
	kal_uint32 status = STATUS_OK;
	kal_uint32 enable = *(kal_uint32*)(data);

	bq24296_set_en_hiz(0x0); //disable hiz

    if(1 == enable){
		bq24296_set_watchdog(0x0); //disable watchdog time.
		bq24296_set_batfet_disable(0x1); //disable BATFET
    }else{
		bq24296_set_watchdog(0x1); //enable watchdog time.
		bq24296_set_batfet_disable(0x0); //enable BATFET
    }
