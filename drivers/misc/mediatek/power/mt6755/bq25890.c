...
void bq25890_set_batfet_dis(unsigned char val)
{
	unsigned int ret = 0;

	ret = bq25890_config_interface((unsigned char) (bq25890_CON9),
				       (unsigned char) (val),
				       (unsigned char) (BATFET_DIS_MASK),
				       (unsigned char) (BATFET_DIS_SHIFT)
	    );
}

void bq25890_set_batfet_dly(unsigned char val)
{
	unsigned int ret = 0;

	ret = bq25890_config_interface((unsigned char) (bq25890_CON9),
				       (unsigned char) (val),
				       (unsigned char) (BATFET_DLY_MASK),
				       (unsigned char) (BATFET_DLY_SHIFT)
	    );
}
...
unsigned int set_shipping_mode_enable(void *data)
{
	...
	bq25890_set_batfet_dly(0x1);
    if(enable){
		bq25890_set_batfet_dis(0x1); //disable BATFET
    }else{
		bq25890_set_batfet_dis(0x0); //enable BATFET
    }
	return status;
}
...
void bq25890_dump_register(void)
{
	...
	#else //optimize the log show structure
	if (Enable_BATDRV_LOG >= BAT_LOG_CRTI) {
		bq25890_ADC_start(1);
		printk("[bq25890 reg@]");
		for (i = 0; i < bq25890_REG_NUM; i++) {
			bq25890_read_byte(i, &bq25890_reg[i]);
			printk("[0x%x]=0x%x ", i, bq25890_reg[i]);
		}
		printk("[bq25890 reg dump end]\n");
	}
	#endif 
	bq25890_ADC_start(1);
