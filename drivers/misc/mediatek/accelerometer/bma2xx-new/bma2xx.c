...
static const struct of_device_id accel_of_match[] = {
	{.compatible = "mediatek,gsensor-bma251"},
	{},
};
#endif
...
static int is_acc_calibrate_data_valid(int cali_value[BMA2XX_AXES_NUM])
{
	// cali_value's unit is mg.
	int offset = (BMA2XX_MAXZ * 3) / 9.8;
...
static ssize_t acc_calibrate_read_proc(struct file *file,	char __user *buffer, size_t count, loff_t *offset)
{
	...
	{
		res = BMA2XX_SetPowerMode(bma2xx_i2c_client, true);
		if(res)
		{
    ...
    {
		res = BMA2XX_ReadData(bma2xx_i2c_client, rawacc);
		if(res < 0)
		{        
	...
	cali_data[obj->cvt.map[BMA2XX_AXIS_Z]] = BMA2XX_MAXZ - obj->cvt.sign[BMA2XX_AXIS_Z] * rawacc[BMA2XX_AXIS_Z];
	...
	BMA2XX_ResetCalibration(bma2xx_i2c_client);
	BMA2XX_WriteCalibration(bma2xx_i2c_client, cali_data);
	
	cnt = sprintf(buffer, "%d %d %d", cali_data[BMA2XX_AXIS_X], cali_data[BMA2XX_AXIS_Y], cali_data[BMA2XX_AXIS_Z]);
...
static ssize_t acc_calibrate_write_proc(struct file *file, const char __user *user_buf, size_t len, loff_t *offset)
{
	...
	GSE_LOG("[x:%d,y:%d,z:%d], copyLen=%d\n", cali_data[BMA2XX_AXIS_X],
		cali_data[BMA2XX_AXIS_Y],cali_data[BMA2XX_AXIS_Z], (int)copyLen);
