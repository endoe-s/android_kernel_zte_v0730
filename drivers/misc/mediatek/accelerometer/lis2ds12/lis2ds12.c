...
//liupeng@wind-mobi.com end
#define LIS2DS12_STEP_COUNTER 		1
#define LIS2DS12_SIGNIFICANT_MOTION 0 //dependent on LIS2DS12_STEP_COUNTER
#define LIS2DS12_TILT_FUNC			0
//add by yanglimin for cycle count,20160919
#define LIS2DS12_STEP_CYCLE_NUMBER    10000

#if (LIS2DS12_STEP_COUNTER || LIS2DS12_SIGNIFICANT_MOTION || LIS2DS12_TILT_FUNC)
#define LIS2DS12_EMBEDED_FUNC
// interrupt configuration, need to modify it by CTM's platform.
//#define GPIO_LIS2DS12_EINT_PIN 			GPIO_GSE_1_EINT_PIN			//eint gpio pin num
//#define GPIO_LIS2DS12_EINT_PIN_M_EINT 	GPIO_GSE_1_EINT_PIN_M_EINT	//eint mode
//#define CUST_EINT_LIS2DS12_NUM 			CUST_EINT_GSE_1_NUM			//eint num
//#define CUST_EINT_LIS2DS12_DEBOUNCE_CN 	CUST_EINT_GSE_1_DEBOUNCE_CN 	//debounce time
//#define CUST_EINT_LIS2DS12_TYPE 		CUST_EINT_GSE_1_NUM			//eint trigger type
...
#define ACC_CALIBRATE_SAMPLE_COUNT  10
static struct proc_dir_entry *acc_calibrate_proc_file = NULL;
static struct {
    s16 x[ACC_CALIBRATE_SAMPLE_COUNT];
    s16 y[ACC_CALIBRATE_SAMPLE_COUNT];
    s16 z[ACC_CALIBRATE_SAMPLE_COUNT];
    s16 x_avg, y_avg, z_avg;
    s16 x_sum, y_sum, z_sum;
    s16 x_offset, y_offset, z_offset;
}acc_calibrate_temp_data;
//Read ps and acc calibrate data from ztecfg in ltr559.c when startup
extern struct ztecfg_data sensor_calibrate_data;
static int is_acc_calibrate_data_valid(int x_offset,int y_offset,int z_offset);
...
//	struct pinctrl_state *pins_default;
	...
	if (IS_ERR(lis2ds12_pinctrl1)) {
		...
		GSE_LOG("fwq Cannot find lis2ds12 lis2ds12_pinctrl1!\n");
		return ret;
	}
	...
	if (IS_ERR(pins_eint_int1)) {
		...
		GSE_LOG("fwq Cannot find lis2ds12  pinctrl state_eint_lis2ds12!!\n");
		return ret;
	}
	pinctrl_select_state(lis2ds12_pinctrl1, pins_eint_int1);
    #endif

	/*node = of_find_matching_node(node, lis2ds12_of_match);*/
	//node = of_find_matching_node(node, accel_of_match);
	node = of_find_compatible_node(NULL,NULL,"mediatek, GSE_1-eint");
//	printk("[lis2ds12]liupeng  fail \n");
	if (node) {
	//	printk("[lis2ds12]liupeng  yes \n");
    	GSE_LOG("interrupt node ok\n");
        ...
        ret = request_irq(accl_irq, lis2ds12_eint_func, IRQF_TRIGGER_HIGH, "mediatek, GSE_1-eint", NULL);
		//disable_irq(accl_irq); // for test
		if (ret != 0) {
			printk("[lis2ds12]EINT IRQ LINE NOT AVAILABLE\n");
		} else {
			printk("[lis2ds12]lis2ds12 set EINT finished, accl_irq=%d, acceldebounce=??\n",
				     accl_irq);
		}
	} else {
		printk("[lis2ds12]%s can't find compatible node\n", __func__);
	}
	return 0;
/*
...
static void LIS2DS12_EintWork(struct work_struct *work)
{
    ...
    //yanglimin add for overflow,20160602 start
	LIS2DS12_Get_Pedo_DataReg(obj->client, &pedo_data);
    GSE_LOG("pedo_data=%d\n", pedo_data);	
    if(pedo_data == 0)
    {
        obj_i2c_data->backup_step += 65536;
    }
    //yanglimin add for overflow,20160602 end
...
static int LIS2DS12_Enable_Tilt_Func_On_Int(struct i2c_client *client, bool enable)
{
	...
	if(enable)
	{
		...
		databuf[0] |= LIS2DS12_INT2_TILT_MASK;			
	}
	else
	{
		databuf[0] &= ~LIS2DS12_INT2_TILT_MASK;//clear 
...
static int LIS2DS12_Reset_Pedo_Data(struct i2c_client *client)
{
    ...
    LIS2DS12_Get_Pedo_DataReg(client, &pedo_data);
    GSE_LOG("backup_step=%d, pedo_data=%d before reset\n", obj_i2c_data->backup_step, pedo_data);
    if(pedo_data > 0)
    {
        obj_i2c_data->backup_step += pedo_data; 
    }
	...
	if (pedo_data > 0)
	{
	    //wait for clear LSM6DS3_STEP_COUNTER_L/H
	    msleep(50);
	}
...
static int LIS2DS12_enable_pedo(struct i2c_client *client, bool enable)
{
	...
	#if 1
		res = LIS2DS12_Write_PedoParameters(client, 1, 0x4b, 0x5f);// set threshold to a certain value here
		if(res != LIS2DS12_SUCCESS)
		{
...
static int lis2ds12_reset_for_cycle_count(struct i2c_client *client)
{
	u8 databuf[2] = {0};
	int res = 0;
	GSE_FUN();
...
static int lis2ds12_step_c_get_data(uint32_t *value, int *status)
{
    ...
    //add by yanglimin for cycle counter,20160919, start
    if(pedo_data >= LIS2DS12_STEP_CYCLE_NUMBER)
    {
        GSE_LOG("cycle count, pedo_data=%d\n", pedo_data);
        obj_i2c_data->backup_step += pedo_data; 
        current_step = obj_i2c_data->backup_step;

        lis2ds12_reset_for_cycle_count(priv->client);
        //LIS2DS12_Get_Pedo_DataReg(priv->client, &pedo_data);
        //GSE_LOG("cycle count,222222, pedo_data=%d\n", pedo_data);
    }
    //add by yanglimin for cycle counter,20160919, end
    else
    {
        current_step = pedo_data + obj_i2c_data->backup_step;
    }
    
    GSE_LOG("current_step=%d, pedo_data=%d, backup_step=%d\n", current_step, pedo_data, obj_i2c_data->backup_step);
...
static int LIS2DS12_ReadData(struct i2c_client *client, char *buf, int bufsize)
{
	...
	else
	{
        ...
        acc[obj->cvt.map[LIS2DS12_AXIS_X]] = - obj->cvt.sign[LIS2DS12_AXIS_X]*obj->data[LIS2DS12_AXIS_X];//yanglimin mod,because X-direction is wrong
...
static int LIS2DS12_ReadRawData(struct i2c_client *client, s16 data[LIS2DS12_ACC_AXES_NUM])
{
	...
	else
	{
		...
		else
		{
            ...
            if(is_acc_calibrate_data_valid(sensor_calibrate_data.acc_offset_x, sensor_calibrate_data.acc_offset_y, sensor_calibrate_data.acc_offset_z))
            {
                data[LIS2DS12_AXIS_X] -= sensor_calibrate_data.acc_offset_x;
                data[LIS2DS12_AXIS_Y] -= sensor_calibrate_data.acc_offset_y;
                data[LIS2DS12_AXIS_Z] -= sensor_calibrate_data.acc_offset_z;
            }
            else
            {
                GSE_ERR("acc offset invalid, xyz offset: %d, %d, %d\n", sensor_calibrate_data.acc_offset_x, sensor_calibrate_data.acc_offset_y, sensor_calibrate_data.acc_offset_z);
            }
            //GSE_ERR("xyz raw data: %d, %d, %d\n", data[LIS2DS12_AXIS_X], data[LIS2DS12_AXIS_Y], data[LIS2DS12_AXIS_Z]);
		}
	}
	return err;
}
...
static ssize_t show_reg_value(struct device_driver *ddri, char *buf)
{
    ...
    {
      //dump all
        hwmsen_read_byte(obj_i2c_data->client,addr,&regdata);
    	GSE_LOG("Reg addr=%x regdata=%x\n",addr,regdata);
        count += snprintf(buf+count, PAGE_SIZE, "%02X,", regdata);
        if((i & 0XF) == 0x0F)
        {
...
static DRIVER_ATTR(dumpreg,      S_IRUGO | S_IWUSR, show_reg_value, NULL);
...
//Add soft reset by yanglimin,because pedo reg is not clear when reboot , start
static int LIS2DS12_Soft_Reset(struct i2c_client *client)
{
	u8 databuf[2] = {0};    
	int res = 0;
	GSE_FUN(); 
...
//Add soft reset by yanglimin,because pedo reg is not clear when reboot , end

//Add by yanglimin for overflow interrupt, start
static int LIS2DS12_Set_Overflow_Int(struct i2c_client *client)
{
   	u8 databuf[2] = {0};    
	int res = 0;

	databuf[1] = 0x10;
	databuf[0] = LIS2DS12_FIFO_CTRL; 
	res = i2c_master_send(client, databuf, 0x2);
	if(res <= 0)
	{
		GSE_ERR("write full scale register err!\n");
    ...
    //Add by yanglimin for latch-mode interrupt,20160922 start
	databuf[1] = 0x04;
	databuf[0] = LIS2DS12_CTRL3; 
	res = i2c_master_send(client, databuf, 0x2);
	if(res <= 0)
	{
		GSE_ERR("write full scale register err!\n");
    ...
    //Add by yanglimin for latch-mode interrupt,20160922 end
    
    /*
    //Enable step detect interrupt,for interrupt test
	databuf[1] = 0x04;
	databuf[0] = LIS2DS12_CTRL5; 
	res = i2c_master_send(client, databuf, 0x2);
	if(res <= 0)
	{
		GSE_ERR("write full scale register err!\n");
...
//Add by yanglimin for overflow interrupt, end
/*----------------------------------------------------------------------------*/
static int LIS2DS12_InitClient(struct i2c_client *client, bool enable)
{
	struct lis2ds12_i2c_data *obj = i2c_get_clientdata(client);
	...
	res = LIS2DS12_Set_Overflow_Int(client);
	if(res != LIS2DS12_SUCCESS)
	{
		return res;
	}
...
/*----------------------------------------------------------------------------
Phone is horizontal when we adjust acc,so (X\Y offset maybe < 1),(Z offset maybe < 2)
yanglimin add for P650F31
----------------------------------------------------------------------------*/
static int is_acc_calibrate_data_valid(int x_offset,int y_offset,int z_offset)
{
//  (gravity value) 1 ~= 210 (reg_data), check phone whether horizontal
    if((abs(x_offset) > 210) || (abs(y_offset) > 210) || (abs(z_offset) > 420))
    {
        return 0;
    }
...
static void acc_calibrate_work(struct work_struct *work)
{
	GSE_LOG("%s, write calibrate data\n", __FUNCTION__);
    write_params_to_ztecfg(TYPE_ACC, acc_calibrate_temp_data.x_offset, acc_calibrate_temp_data.y_offset, acc_calibrate_temp_data.z_offset);
}
static ssize_t acc_calibrate_read_proc(struct file *file,	char __user *buffer, size_t count, loff_t *offset)
{
    ...
    memset(&acc_calibrate_temp_data, 0, sizeof(acc_calibrate_temp_data));
	if(NULL == lis2ds12_i2c_client)
	{
		err = -EINVAL;
        GSE_ERR("lis2ds12_i2c_client NULL\n");
        goto fail;
	}
	else
	{
        for(i=0; i<ACC_CALIBRATE_SAMPLE_COUNT; i++)
        {
    		if(hwmsen_read_block(lis2ds12_i2c_client, LIS2DS12_OUTX_L_XL|AUTO_INCREMENT, databuf, LIS2DS12_ACC_DATA_LEN))
    		{
    			GSE_ERR("LIS2DS12 read acc data  error\n");
                goto fail;
    		}
    		else
    		{
    			...
    			data[LIS2DS12_AXIS_Z] = ((s16)(((databuf[LIS2DS12_AXIS_Z*2+1] << 8) | databuf[LIS2DS12_AXIS_Z*2])) >> 2);
                //GSE_ERR("xyz raw data: %d, %d, %d\n", data[LIS2DS12_AXIS_X], data[LIS2DS12_AXIS_Y], data[LIS2DS12_AXIS_Z]);
                acc_calibrate_temp_data.x[i] = data[LIS2DS12_AXIS_X];
                acc_calibrate_temp_data.y[i] = data[LIS2DS12_AXIS_Y];
                acc_calibrate_temp_data.z[i] = data[LIS2DS12_AXIS_Z];

                acc_calibrate_temp_data.x_sum += acc_calibrate_temp_data.x[i];
                acc_calibrate_temp_data.y_sum += acc_calibrate_temp_data.y[i];
                acc_calibrate_temp_data.z_sum += acc_calibrate_temp_data.z[i];
    		}
            mdelay(10);
        }
        acc_calibrate_temp_data.x_avg = acc_calibrate_temp_data.x_sum / ACC_CALIBRATE_SAMPLE_COUNT;
        acc_calibrate_temp_data.y_avg = acc_calibrate_temp_data.y_sum / ACC_CALIBRATE_SAMPLE_COUNT;
        acc_calibrate_temp_data.z_avg = acc_calibrate_temp_data.z_sum / ACC_CALIBRATE_SAMPLE_COUNT;
	}
    //offset = raw_data - standard_value
    acc_calibrate_temp_data.x_offset = acc_calibrate_temp_data.x_avg - 0;
    acc_calibrate_temp_data.y_offset = acc_calibrate_temp_data.y_avg - 0;
    acc_calibrate_temp_data.z_offset = acc_calibrate_temp_data.z_avg - 2048;

    GSE_LOG("x,y,z offset: %d, %d, %d\n", acc_calibrate_temp_data.x_offset, acc_calibrate_temp_data.y_offset, acc_calibrate_temp_data.z_offset);

    //  1 ~= 210 reg_data, check phone whether horizontal
    if(0 == is_acc_calibrate_data_valid(acc_calibrate_temp_data.x_offset, acc_calibrate_temp_data.y_offset, acc_calibrate_temp_data.z_offset))
    {
        GSE_LOG("calibrate data invalid, phone maybe not horizontal\n");
        goto fail;
    }
    if(NULL != obj_i2c_data)
    {
        schedule_work(&obj_i2c_data->calibrate_work);
    }
    else
    {
        GSE_LOG("obj_i2c_data NULL\n");
        goto fail;
    }
    sensor_calibrate_data.acc_offset_x = acc_calibrate_temp_data.x_offset;
    sensor_calibrate_data.acc_offset_y = acc_calibrate_temp_data.y_offset;
    sensor_calibrate_data.acc_offset_z = acc_calibrate_temp_data.z_offset;

	cnt = sprintf(buffer, "1\n");
    *offset += cnt;
    GSE_LOG("%s,ok\n",__func__);
    return cnt;
...
static void create_acc_calibrate_proc_file(void)
{
	...
	{
	    printk(KERN_ERR "create_acc_calibrate_proc_file fail!\n");
	}
}
...
#endif
	INIT_WORK(&obj->calibrate_work, acc_calibrate_work);
	obj->hw = get_cust_acc(); 
