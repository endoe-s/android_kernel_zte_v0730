...
void spm_pmic_power_mode(int mode, int force, int lock)
{
	...
	case PMIC_PWR_SUSPEND:
		...
		/* ldo control */
		//pmic_ldo_vldo28_sw_en(0);    //for tp bsg
...
//leon 20160624 mod for ALPS02761394
/*#if 0
	pr_debug("vmd_sel: addr = 0x%x, bit = %d\n", gpio_dout_addr, gpio_dout_bit);
#endif*/
#if 1
       pr_err("vmd_sel: addr = 0x%x, bit = %d", gpio_dout_addr, gpio_dout_bit);
       pr_debug("vmd_sel: addr = 0x%x, bit = %d,segment = 0x%x", gpio_dout_addr, gpio_dout_bit,segment);
#endif
