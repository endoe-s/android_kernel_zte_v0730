...
extern  void red_blink_add(void);

static ssize_t show_red_blink(struct device *dev, struct device_attribute *attr, char *buf)
{
	return 0;
}


static ssize_t store_red_blink(struct device *dev, struct device_attribute *attr,
				  const char *buf, size_t size)
{
...
static DEVICE_ATTR(red_blink, 0664, show_red_blink, store_red_blink);
...
static int mt65xx_leds_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < MT65XX_LED_TYPE_TOTAL; i++) {
        ...
        {
           device_create_file(g_leds_data[i]->cdev.dev, &dev_attr_red_blink);
        }
        /*2016.6.12 add ---e*/
