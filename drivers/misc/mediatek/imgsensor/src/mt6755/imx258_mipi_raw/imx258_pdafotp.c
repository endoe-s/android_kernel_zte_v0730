...
#define IMX258_EEPROM_WRITE_ID  0xA0
#define IMX258_EEPROM_READ_ID   0xA1
...
static bool selective_read_eeprom(kal_uint16 addr, BYTE* data)
{
	...
	if(iReadRegI2C(pu_send_cmd, 2, (u8*)data, 1, IMX258_EEPROM_WRITE_ID)<0)
		return false;
    return true;
}
...
static bool _read_imx258_eeprom(kal_uint16 addr, BYTE* data, kal_uint32 size ){
	...
	for(i = 0; i < size; i++) {
		...
		printk("read_eeprom [0x%0x] =0x%0x---lhl-0613-333-- \n",offset, data[i]);
		offset++;
	}
	get_done = true;
	last_size = size;
...
bool read_imx258_eeprom( kal_uint16 addr, BYTE* data, kal_uint32 size)
	...
	printk("read imx258 eeprom PDAF, size = %d---lhl---\n", size);
...
bool read_imx258_eeprom_SPC( kal_uint16 addr, BYTE* data, kal_uint32 size){
	...
	printk("read imx258 eeprom SPC, size = %d\n", size);
