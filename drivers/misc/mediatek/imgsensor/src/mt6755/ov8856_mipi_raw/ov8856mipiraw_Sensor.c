...
extern void ov8856_otp_cali(unsigned char writeid);
...
static SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[5] =	 
{
 ...
 #endif
 { 3264, 2448,	  0,	0, 3264, 2448, 1632, 1224,   0,	0, 1632, 1224,	 0, 0, 1632, 1224}, // Preview   lzl20160723
 { 3264, 2448,	  0,	0, 3264, 2448, 3264, 2448,   0,	0, 3264, 2448,	 0, 0, 3264, 2448}, // capture 
 { 3296, 2480,	  12,	12, 3272, 2456, 3264, 2452,   0,	0, 3264, 2448,	 0, 0, 3264, 2448}, // video 
 { 3296, 2480,	  12,	12, 3272, 2456,  640,  480,   0,	0,  640,  480,	 0, 0,  640,  480}, //hight speed video 
 { 3296, 2480,	  12,	12, 3272, 2456, 1636, 1228,   0,	0, 1632, 1224,	 0, 0, 1632, 1224}// slim video 
...
static void sensor_init(void)
{
    ...
    */
	...
	write_cmos_sensor(0x030d, 0x3c);//ZTE gaoyangyang modify for GPS 0x3f,0x40,0x46
	write_cmos_sensor(0x030f, 0x08);//ZTE gaoyangyang modify for GPS 0x05
	write_cmos_sensor(0x0312, 0x03);//ZTE gaoyangyang modify for GPS 0x11
  ...
  write_cmos_sensor(0x3706, 0x86); //neil 20151120
  write_cmos_sensor(0x370b, 0x7e); //neil 20151120
	...
	write_cmos_sensor(0x380e, 0x0a);//ZTE gaoyangyang modify for GPS 0x09
	write_cmos_sensor(0x380f, 0xc8);//ZTE gaoyangyang modify for GPS 0xb2
...
static void preview_setting(void)
{
	...
	write_cmos_sensor(0x380d, 0x8c);
	write_cmos_sensor(0x380e, 0x0a);//ZTE gaoyangyang modify for GPS 0x09
	write_cmos_sensor(0x380f, 0xc8);//ZTE gaoyangyang modify for GPS 0xb0
	write_cmos_sensor(0x3810, 0x00);
	...
	write_cmos_sensor(0x3815, 0x01);
	write_cmos_sensor(0x3816, 0x00); //0x04);
	write_cmos_sensor(0x3817, 0x00); //0xde);
	write_cmos_sensor(0x3818, 0x00);
	write_cmos_sensor(0x3819, 0x00); //0x08); for FOV same with snapshot
	write_cmos_sensor(0x3820, 0xd6);
	write_cmos_sensor(0x3821, 0x21);
...
static void capture_setting(kal_uint16 currefps)
{
	...
	if (currefps == 150) {
		...
		write_cmos_sensor(0x4502, 0x50);//improve the normal luminance differences new add for set back to default
		write_cmos_sensor(0x382a, 0x01);
		write_cmos_sensor(0x4009, 0x0b);
		write_cmos_sensor(0x4601, 0x80);
		write_cmos_sensor(0x5795, 0x02);
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			if (*sensor_id == imgsensor_info.sensor_id) {
				if(1){//if((read_cmos_sensor(0x302A)) == 0XB0){
			...
			LOG_INF("Read sensor id fail, id: 0x%x,sensor_id:%x\n", imgsensor.i2c_write_id,*sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		retry = 2;
...
static kal_uint32 open(void)
{
	...
	//	LOG_INF("Apply the sensor OTP\n");
	  ov8856_otp_cali(imgsensor.i2c_write_id);
	//	read_otp(otp_ptr);
	  write_cmos_sensor(0x0100, 0x00);
	//	kfree(otp_ptr);
	#endif
