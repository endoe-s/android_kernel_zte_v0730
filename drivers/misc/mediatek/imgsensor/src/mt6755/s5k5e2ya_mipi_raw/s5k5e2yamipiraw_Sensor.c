...
#define ST55A_USE_AWB_OTP
#define GAIN_DEFAULT       	     0x0100
#define OTP_MID_Qtech            0x06
#define OTP_MID_Seasons          0x10
static kal_uint16 r_ratio = 0x17f;//  0x17f  //  383
static kal_uint16 b_ratio = 0x133;//  0x133  //  307
static kal_uint16 module_id = 0;//  0x133  //  307
...
static void sensor_init(void)
{
   ...
   // +++++++++++++++++++++++++++//
	...
	write_cmos_sensor(0x0101,0x03);       //set mirror,flip
...
static kal_uint16 Agolden_R = 0,  Agolden_G = 0, Agolden_B = 0, Acurrent_R = 0, Acurrent_G = 0,  Acurrent_B = 0;
#ifdef ST55A_USE_AWB_OTP
static void s5k5e2ya_MIPI_read_otp_wb(void)
{
   	kal_uint16 golden_R = 0, 
		       golden_G = 0, 
		       golden_B = 0, 
		       current_R = 0, 
		       current_G = 0,  
		       current_B = 0;
    uint16_t wb_flag = 0,retry=0;
               if(!(!Agolden_G || !Acurrent_G || !Agolden_R || !Agolden_B || !Acurrent_R || !Acurrent_B))
               	{
	               golden_R = Agolden_R; 
	   		       golden_G = Agolden_G; 
	   		       golden_B = Agolden_B;
	   		       current_R = Acurrent_R; 
	   		       current_G = Acurrent_G;  
	   		       current_B = Acurrent_B;
               	}
			   else
			   	{
Rtry_read_otp:
	        	   write_cmos_sensor(0x0a00, 0x04); 
	        	   write_cmos_sensor(0x0a02, 0x02); //page 2
				   write_cmos_sensor(0x0a00, 0x01);		
	               mdelay(100);
				   wb_flag = read_cmos_sensor(0x0A04);
	               if((wb_flag&(1<<7))==0)//group 1   0: Empty/valid;
	               	{
	                 	printk("s5k5e2ya otp use page 2 group 1 ,mid = 0x%x\n ",wb_flag);
						module_id = wb_flag;
		        	      current_R =  read_cmos_sensor(0x0A06);
				       current_G =  read_cmos_sensor(0x0A07);
		        	       current_B =  read_cmos_sensor(0x0A08); 
				      golden_R = 0x73;//golden_R =	read_cmos_sensor(0x0A09);
				      golden_G =0x88 ;//golden_G =	read_cmos_sensor(0x0A0a);
				      golden_B =0x66;//golden_B =	read_cmos_sensor(0x0A0B);					  
	               	}
				   else //read page 3
				   	{ 
						write_cmos_sensor(0x0a00, 0x04); 
						write_cmos_sensor(0x0a02, 0x03); //page 3
						write_cmos_sensor(0x0a00, 0x01); 	
						mdelay(100);
						wb_flag = read_cmos_sensor(0x0A04);
						if((wb_flag&(1<<7))==0)//group 1   0: Empty/valid;
							{
								printk("s5k5e2ya otp use page 3 group 2 ,mid = 0x%x\n ",wb_flag);
								module_id = wb_flag;
								current_R =  read_cmos_sensor(0x0A06);
								current_G =  read_cmos_sensor(0x0A07);
								current_B =  read_cmos_sensor(0x0A08); 
								...
								golden_B =0x66;//golden_B =	read_cmos_sensor(0x0A0B);				  
							      ...
							      {
							      write_cmos_sensor(0x0a00, 0x04); 
						           write_cmos_sensor(0x0a02, 0x04); //page 3
						           write_cmos_sensor(0x0a00, 0x01); 	
						                  mdelay(100);
						             wb_flag = read_cmos_sensor(0x0A04);
						            if((wb_flag&(1<<7))==0)//group 1   0: Empty/valid;
							{
								printk("s5k5e2ya otp use page 4 group 3 ,mid = 0x%x\n ",wb_flag);
								module_id = wb_flag;
								current_R =  read_cmos_sensor(0x0A06);
								current_G =  read_cmos_sensor(0x0A07);
								current_B =  read_cmos_sensor(0x0A08); 
                   ...
                   if(!current_G || !current_R || !current_B)
                   	{
                      retry++;
					  printk("s5k5e2ya otp retry ,retry = %d\n ",retry);
					  if(retry<2)
					  	{
					      goto Rtry_read_otp;
					  	}
					   ...
					   Agolden_G = golden_G; 
					   Agolden_B = golden_B;
					   Acurrent_R = current_R; 
					   Acurrent_G = current_G;	
					   Acurrent_B = current_B;
				   	}
			   }
  		printk("s5k5e2ya otp current_R=0x%x, current_G=0x%x, current_B=0x%x, golden_R=0x%x, golden_G=0x%x, golden_B=0x%x\n", current_R,current_G,current_B,golden_R,golden_G,golden_B);
		write_cmos_sensor(0x0a00, 0x04); 
		write_cmos_sensor(0x0a00, 0x00);
		if(!golden_G || !current_G || !golden_R || !golden_B || !current_R || !current_B)
     		printk("WB update error!");
   		r_ratio = 512 * golden_R * current_G /( golden_G * current_R );
   		b_ratio = 512 * golden_B * current_G /( golden_G * current_B );
   		printk("s5k5e2ya otp  r_ratio=0x%x, b_ratio=0x%x\n", r_ratio, b_ratio);      
}
static void s5k5e2ya_MIPI_algorithm_otp_wb1(void)
{
   kal_uint16 R_GAIN = 0, 
   	          B_GAIN = 0, 
   	          G_GAIN = 0;
	...
	else                      
   {
    ...
    write_cmos_sensor(0x020e, G_GAIN >> 8); //g_gain
    write_cmos_sensor(0x020f, G_GAIN & 0xFFFF); //g_gain
    write_cmos_sensor(0x0210, R_GAIN >> 8); //r_gain
    write_cmos_sensor(0x0211, R_GAIN & 0xFFFF); //r_gain
    write_cmos_sensor(0x0212, B_GAIN >> 8); //b_gain
    write_cmos_sensor(0x0213, B_GAIN & 0xFFFF); //b_gain
    write_cmos_sensor(0x0214, G_GAIN >> 8); //g_gain
    write_cmos_sensor(0x0215, G_GAIN & 0xFFFF); //g_gain
    printk("s5k5e2ya otp R_gain=0x%x, B_gain=0x%x, G_gain=0x%x\n", R_GAIN, B_GAIN, G_GAIN);    
}
#if 0
static void read_lsc_test()
{
              int i=9;
		int j=0x0A04;
		int num=0x0;
			  for (i=9;i<=0xf;i++)
			  	{
				write_cmos_sensor(0x0a00, 0x04); 
	        	       write_cmos_sensor(0x0a02, i); //page 2
				   write_cmos_sensor(0x0a00, 0x01);		
	                     mdelay(100);
				for(j=0x0A04;j<=0x0A43;j++)
					{
					num=read_cmos_sensor(j);
				printk("page = %d, addr=0x%X,  : %x\n",i,j,num);
					}
			  	}   
}
#endif
...
CheckSubid:	
#ifdef ST55A_USE_AWB_OTP
	s5k5e2ya_MIPI_read_otp_wb();
    if(module_id==OTP_MID_Seasons)
    	{
    	   printk("s5k5e2ya otp use OTP_MID_Seasons");
           return ERROR_NONE;
    	}
    else 
    	{
...
#ifdef ST55A_USE_AWB_OTP
    s5k5e2ya_MIPI_read_otp_wb();
    s5k5e2ya_MIPI_algorithm_otp_wb1();
	s5k5e2ya_MIPI_otp_lsc();
#endif
