...
#define LOG_INF(format, args...)	pr_debug("s5k4h8_otp" "%s: " format, __FUNCTION__ , ##args) 
...
#define s5k4h8_write_cmos_sensor(addr, para)  iWriteReg((u16) addr , (u32) para , 1, s5k4h8MIPI_WRITE_ID)//add by hhl
...
#define Golden_RG   0x12F //0x206  //0x209  // modify by gionee wangxiaoyu 2015-09-01
#define Golden_BG   0x101 //0x24B  //0x259  // modify by gionee wangxiaoyu 2015-09-01
...
kal_uint16 s5k4h8_read_cmos_sensor(kal_uint32 addr)
{
	kal_uint16 get_byte=0;
	iReadReg((u16) addr ,(u8*)&get_byte,s5k4h8MIPI_WRITE_ID);
	return get_byte;
}
/*************************************************************************************************
...
void start_read_otp(void)
{
	...
	s5k4h8_write_cmos_sensor(0x0100, 0x01);
	s5k4h8_write_cmos_sensor(0x0A02, 0x0F);   //Select the page to write by writing to 0xD0000A02 0x01~0x0C
	s5k4h8_write_cmos_sensor(0x0A00, 0x01);   //Enter read mode by writing 01h to 0xD0000A00
	Sleep(20);
}
...
{
	s5k4h8_write_cmos_sensor(0x0A00, 0x00);   //Reset the NVM interface by writing 00h to 0xD0000A00
}
...
BYTE get_otp_flag(BYTE zone)
{
	...
	if(zone==1)
	{
		flag = s5k4h8_read_cmos_sensor(0x0A14);

	}
	if(zone==2)
	{
		flag = s5k4h8_read_cmos_sensor(0x0A30);
	...
	else if((flag&0xFF)==0x40)//valid
	{
		return 1;
	}
	else if((flag&0xFF)==0x60)//invalid
	{
		return 2;
	}
	else 
...
bool otp_lenc_update(USHORT zone)
{
	...
	s5k4h8_write_cmos_sensor(0x602A, 0x0b00);
	s5k4h8_write_cmos_sensor(0x6F12, 0x01);
	s5k4h8_write_cmos_sensor(0x6028, 0x2000);

	if(s5k4h8_version==0xb001)
		s5k4h8_write_cmos_sensor(0x602A, 0x0C20);
	else if(s5k4h8_version==0xc001)
...
#if 1
	s5k4h8_write_cmos_sensor(0x020e, G_GainH);
	s5k4h8_write_cmos_sensor(0x020f, G_GainL);
	s5k4h8_write_cmos_sensor(0x0210, R_GainH);
	s5k4h8_write_cmos_sensor(0x0211, R_GainL);
	s5k4h8_write_cmos_sensor(0x0212, B_GainH);
	s5k4h8_write_cmos_sensor(0x0213, B_GainL);
	s5k4h8_write_cmos_sensor(0x0214, G_GainH);
	s5k4h8_write_cmos_sensor(0x0215, G_GainL);
#endif
...
bool get_otp_wb(int zone)
{
	...
	start_read_otp();
	if(zone == 1){
		temph = s5k4h8_read_cmos_sensor(0x0A0C);// + (zone*WB_OFFSET));
		templ = s5k4h8_read_cmos_sensor(0x0A0D);// + (zone*WB_OFFSET));

		Current_RG = (USHORT)((temph<<8)| templ);

		temph = s5k4h8_read_cmos_sensor(0x0A0E);//+ (zone*WB_OFFSET));
		templ = s5k4h8_read_cmos_sensor(0x0A0F);//+ (zone*WB_OFFSET));
