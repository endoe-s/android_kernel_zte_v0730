        ...
        do {
            *sensor_id = return_sensor_id();
            ...
            if (*sensor_id == imgsensor_info.sensor_id) {
               //LOG_INF("ov13853 i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                return ERROR_NONE;
            }
            //LOG_INF("ov13853 Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 2;
...
#if 0
void ov13853_read_otp(unsigned char writeid)
{
	kal_uint32 addr;
	

	for(addr=0; addr<=0xcd8;addr++)
	{
	    kal_uint16 get_byte=0;
	    char pu_send_cmd[2] = {(char)(addr >> 8), (char)(addr & 0xFF) };
	    iReadRegI2C(pu_send_cmd, 2, (u8*)&get_byte, 1, 0xA0);
	   printk("--zlx--ov13853_read_otp:[0x%x]=[0x%x]\n", addr, get_byte);	
	}
 ...
 //   unsigned long long *feature_return_para=(unsigned long long *) feature_para;
    ...
    switch (feature_id) {
        ...
        case SENSOR_FEATURE_CHECK_SENSOR_ID:
			...
			{
			   extern int DW9767AF_Release_First(void);
		       DW9767AF_Release_First();
        	}
			#endif
			//end
