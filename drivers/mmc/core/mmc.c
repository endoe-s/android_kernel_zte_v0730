...
static int emmc_id_proc_show(struct seq_file *m, void *v)
{
 ...
 seq_printf(m, 
		          ...
		          card->cid.prod_name,
		          card->cid.month, card->cid.year, 
		         (card->ext_csd.sectors) /2097152);      //GB
...
static int ddr_id_proc_show(struct seq_file *m, void *v)
{
 ...
 if(card->ext_csd.sectors <16000000) 
			 seq_printf(m, 
		          "%s?NA?NA?1GB?LPDDR3\n",
		          manfname);
else if( card->ext_csd.sectors <32000000) 
			 seq_printf(m, 
		          "%s?NA?NA?2GB?LPDDR3\n",
		          manfname);
else
...
int mmc_attach_mmc(struct mmc_host *host)
{
	...
	int i;
#endif
       static bool emmc_proc_init = false; // [ECID:000000] ZTEBSP wangqiaoming record eMMC info into /proc/driver/emmc file, 20131108
	BUG_ON(!host);
	WARN_ON(!host->claimed);
