...
{
	int ret = 0;
	kal_bool charging_state=KAL_FALSE;
	...
	case POWER_SUPPLY_PROP_CURRENT_NOW:
		battery_current =battery_meter_get_battery_current();
		battery_current = (battery_current/10)*1000;
		charging_state=battery_meter_get_battery_current_sign();
		if(charging_state==KAL_TRUE){
		    val->intval = 0 - battery_current;
		}
		else{
...
extern unsigned int set_shipping_mode_enable(void *data);
static ssize_t show_bq24296_shipping_mode(struct device *dev,struct device_attribute *attr,
					char *buf)
{
    return sprintf(buf, "%u\n", 0);
}
static ssize_t store_bq24296_shipping_mode(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    int enable = 0;
	sscanf(buf, "%d", &enable);
    battery_log(BAT_LOG_CRTI, "shipping_mode= %d\n",enable);

	set_shipping_mode_enable(&enable);
	
    return size;
}
static DEVICE_ATTR(BQ24296_SHIPPING_MODE, 0664, show_bq24296_shipping_mode, store_bq24296_shipping_mode);
#ifdef NEED_DISABLE_CHARGING_IN_META_MODE
BOOL charging_enable_in_meta_mode  = KAL_FALSE;
static ssize_t show_disable_charging_in_meta_mode(struct device *dev,struct device_attribute *attr,
					char *buf)
{
...
static ssize_t store_disable_charging_in_meta_mode(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    int enable = 0;
	sscanf(buf, "%d", &enable);
    battery_log(BAT_LOG_CRTI, "enable= %d\n",enable);
	if(1==enable)
	{
		g_platform_boot_mode = NORMAL_BOOT;
		charging_enable_in_meta_mode = KAL_TRUE;
		battery_log(BAT_LOG_CRTI, "enable charging in meta mode");
	}
	else
	{	
		g_platform_boot_mode = META_BOOT;
		charging_enable_in_meta_mode = KAL_FALSE;
		battery_log(BAT_LOG_CRTI, "disable charging in meta mode");
	}
	
	wake_up_bat();
    return size;
}
static DEVICE_ATTR(DISABLE_CHARGING_IN_META_MODE, 0664, show_disable_charging_in_meta_mode, store_disable_charging_in_meta_mode);
#endif
#endif


static ssize_t show_rtc_fg_soc(struct device *dev,struct device_attribute *attr,
					char *buf)
{
	int rtc_fg_soc = -1;
...
static ssize_t store_rtc_fg_soc(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    return size;
}
static DEVICE_ATTR(rtc_fg_soc, 0664, show_rtc_fg_soc, store_rtc_fg_soc);
...
static kal_bool mt_battery_100Percent_tracking_check(void)
{
	...
	if (BMT_status.bat_full == KAL_TRUE) {	/* charging full first, UI tracking to 100% */
		if (BMT_status.UI_SOC >= 100) {
			...
			//	return resetBatteryMeter; //防止充电芯片报满，而UI_SOC不满，电量出现跳变
			}
...
static void battery_update(struct battery_data *bat_data)
{
	...
	if ((BMT_status.charger_exist == KAL_TRUE) && (BMT_status.bat_charging_state != CHR_ERROR)) {
		if (BMT_status.bat_exist) {	/* charging */
			...
			/*add for quick charging--s*/
            #if defined(CONFIG_MTK_PUMP_EXPRESS_PLUS_SUPPORT) || defined(CONFIG_MTK_PUMP_EXPRESS_SUPPORT) || defined(CONFIG_MTK_PUMP_EXPRESS_PLUS_20_SUPPORT)
			if ((KAL_TRUE == is_pe20_connect)||(KAL_TRUE == is_ta_connect)) {
				bat_data->BAT_STATUS = POWER_SUPPLY_STATUS_CMD_DISCHARGING + 1;
			}
			#endif 
			/*add for quick charging --e*/
...
void mt_battery_GetBatteryData(void)
{
	...
	battery_current = battery_meter_get_battery_current(); //add
	if (battery_meter_get_battery_current_sign() == KAL_TRUE)
	{
		battery_current = 0 - battery_current;
	}
...
#if 1 //for excel
battery_log(BAT_LOG_CRTI,
"AvgVbat=\t%d\t,bat_vol=\t%d\t,AvgIchr=\t%d\t,Ichr=\t%d\t,Ibat=\t%d\t,VChr=\t%d\t,AvgT=\t%d\t,T=\t%d\t,pre_SOC=\t%d\t,SOC=\t%d\t,UI_SOC=\t%d\t,ZCV=\t%d\t,CHR_Type=\t%d\t\n",
BMT_status.bat_vol, bat_vol, BMT_status.ICharging, ICharging, battery_current/10,BMT_status.charger_vol,
BMT_status.temperature, temperature, previous_SOC, BMT_status.SOC,BMT_status.UI_SOC, BMT_status.ZCV, BMT_status.charger_type);
#else
battery_log(BAT_LOG_CRTI,
...
BMT_status.temperature, temperature, previous_SOC, BMT_status.SOC, BMT_status.ZCV);
#endif 
}
...
static PMU_STATUS mt_battery_CheckChargerVoltage(void)
{
	...
	if (BMT_status.charger_exist == KAL_TRUE) {
		...
		{/*2016.7.7 add */
			...
			battery_log(BAT_LOG_CRTI, "[BATTERY] resume from Charger over voltage !!\r\n");
...
extern void chr_current_auto_adjust_variable_init(void);
#endif
void do_chrdet_int_task(void)
{
	if (g_bat_init_flag == KAL_TRUE) {
		...
		if ((DISO_data.diso_state.cur_vusb_state == DISO_ONLINE) ||
			...
			wake_lock(&battery_suspend_lock);
#ifdef	MTK_CHARGING_CURRENT_AUTO_ADJUST
			chr_current_auto_adjust_variable_init();
#endif 
...
void BAT_thread(void)
{
	...
	if((g_platform_boot_mode == META_BOOT)||(g_platform_boot_mode ==ADVMETA_BOOT))
	{
		if(BMT_status.SOC <=5)
		{
			printk("in meta mode battery soc<=5,shutdown! ");
			mt_power_off();
		}
	}
...
#if (defined(CONFIG_MTK_BQ24296_SUPPORT)||defined(CONFIG_MTK_BQ25896_SUPPORT))
		ret_device_file = device_create_file(&(dev->dev), &dev_attr_BQ24296_SHIPPING_MODE);
#ifdef NEED_DISABLE_CHARGING_IN_META_MODE
		ret_device_file = device_create_file(&(dev->dev), &dev_attr_DISABLE_CHARGING_IN_META_MODE);
#endif
#endif
		ret_device_file = device_create_file(&(dev->dev), &dev_attr_rtc_fg_soc);
