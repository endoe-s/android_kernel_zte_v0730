...
static signed int mtk_pe20_retry_increase(CHR_VOLTAGE_ENUM chr_vol)
{
	...
	if ((retransmit_count == PE20_RETRY_TIMES) ||( BMT_status.charger_exist == KAL_FALSE) || (pe20_cable_out_occur == KAL_TRUE))
		return STATUS_FAIL;
...
static void battery_pump_express20_algorithm_start(void)
{
	...
	if (g_bcct_flag) {
		mtk_pe20_reset_vchr();
		battery_log(BAT_LOG_CRTI,"[PE+20],reset vchr when bcct\n");
		return;
	}
	/*2016.6.16 add --e*/
	...
	if (BMT_status.SOC > 85 && current_sign == KAL_TRUE && charging_current < 10000 && pe20_cv_state == KAL_FALSE) {
		...
		for (i = 0; i < size; i++) {
			...
			if (tune != 0) {
				...
				switch_charger_set_vindpm(CHR_VOLT_04_500000_V); //avoid the system voltage droping when battery full
			}
			break;
		}
		pes = 2;
...
void set_pe20_charging_current(void)
{
	...
	g_temp_input_CC_value = TA20_AC_INPUT_CHARGING_CURRENT;
	g_temp_CC_value = TA20_AC_CHARGING_CURRENT;
...
static kal_bool mtk_ta_retry_increase(void)
{
	...
	if ((retransmit_count == 3) || (BMT_status.charger_exist == KAL_FALSE) || (ta_cable_out_occur == KAL_TRUE))
		return KAL_FALSE;
...
#if 1	/* move them to pchr_turn_on_charging()*/
...
*/
	/* set jeita chaging Current for different tempteture*/
	if (g_temp_status == TEMP_POS_0_TO_POS_10) {
		if (g_temp_CC_value > JEITA_CURRENT_0_10_MAX) {
			g_temp_CC_value = JEITA_CURRENT_0_10_MAX - JEITA_CURRENT_DELTA;
		}
	}else if (g_temp_status == TEMP_POS_10_TO_POS_45) {
		if (g_temp_CC_value > JEITA_CURRENT_10_45_MAX) {
			g_temp_CC_value = JEITA_CURRENT_10_45_MAX - JEITA_CURRENT_DELTA;
		}
	}else if (g_temp_status == TEMP_POS_45_TO_POS_60) {
		if (g_temp_CC_value > JEITA_CURRENT_45_60_MAX) {
			g_temp_CC_value = JEITA_CURRENT_45_60_MAX - JEITA_CURRENT_DELTA;
		}
	}
	battery_log(BAT_LOG_CRTI, "[BATTERY] JEITA set charging current : %d\r\n",
				g_temp_CC_value);
...
unsigned int set_chr_input_current_limit(int current_limit)
{
	...
	if (current_limit != -1) {
		...
		battery_log(BAT_LOG_CRTI, "[BATTERY] set_chr_input_current_limit current_limit(%d), g_temp_input_CC_value(%d), g_bcct_input_value(%d),CHRType(%d)  \r\n",
		current_limit, g_temp_input_CC_value, g_bcct_input_value, BMT_status.charger_type);
...
#ifdef	MTK_CHARGING_CURRENT_AUTO_ADJUST
extern unsigned int mt_get_bl_brightness(void);
extern int  bq24296_is_dpm_mode(void);
static int dpm_mode_cnt  = 0 ;
static int charger_first_plug_in_delay = 0;
static int current_index = CHG_CURR_AUTO_ADJUST_MAX_INDEX-1;
int charging_current_auto_adjust_array[CHG_CURR_AUTO_ADJUST_MAX_INDEX] = {CHARGE_CURRENT_1000_00_MA, CHARGE_CURRENT_1450_00_MA, CHARGE_CURRENT_1800_00_MA};
static int backlight_off_delay_time  = 0; 
static int auto_adjust_completed_current_index = 0;
static unsigned int auto_adjust_completed_cnt = 0;
void chr_current_auto_adjust_variable_init(void)
{
	current_index  = CHG_CURR_AUTO_ADJUST_MAX_INDEX-1;
	dpm_mode_cnt = 0 ;
	charger_first_plug_in_delay = 0;
	auto_adjust_completed_current_index = 0;
	auto_adjust_completed_cnt = 0;
}
#endif
#ifdef NEED_DISABLE_CHARGING_IN_META_MODE 
#if (defined(CONFIG_MTK_BQ24296_SUPPORT)||defined(CONFIG_MTK_BQ25896_SUPPORT))
extern BOOL charging_enable_in_meta_mode ;
#endif
#endif
static void pchr_turn_on_charging(void)
{
#if 1	//!defined(CONFIG_MTK_JEITA_STANDARD_SUPPORT)
	BATTERY_VOLTAGE_ENUM cv_voltage;
#endif
	...
	if (BMT_status.bat_charging_state == CHR_ERROR) {
		...
		battery_charging_control(CHARGING_CMD_INIT, NULL); //add
		#ifdef NEED_DISABLE_CHARGING_IN_META_MODE
		if(charging_enable_in_meta_mode == KAL_TRUE)
		{
			charging_enable = KAL_TRUE;
			select_charging_current();
			battery_log(BAT_LOG_FULL, "[META_BATTERY] enable charging in meta mode!select_charging_curret !\n");
...
#ifdef	MTK_CHARGING_CURRENT_AUTO_ADJUST
	else if((charger_first_plug_in_delay==1)
			&&(mt_get_bl_brightness()== 0)
			&&(BMT_status.SOC < STOP_CHARGING_CURRENT_AUTO_ADJUST_BATTERY_SOC)
		    &&(BMT_status.charger_type == STANDARD_CHARGER)){
			if(backlight_off_delay_time == 1){
				battery_charging_control(CHARGING_CMD_INIT, NULL);
				g_temp_CC_value = charging_current_auto_adjust_array[current_index];
				g_temp_input_CC_value = CHARGE_CURRENT_3000_00_MA;
				#if defined(CONFIG_MTK_JEITA_STANDARD_SUPPORT)
				set_jeita_charging_current();
				#endif
				battery_charging_control(CHARGING_CMD_SET_INPUT_CURRENT,
									&g_temp_input_CC_value);
				battery_charging_control(CHARGING_CMD_SET_CURRENT, &g_temp_CC_value);
				if(bq24296_is_dpm_mode())
				{	
					dpm_mode_cnt++;
					printk("bq24298 is in dpm mode,dpm_mode_cnt=%d!!!\r\n",dpm_mode_cnt);
				}
				else
				{
					dpm_mode_cnt = 0;
				}
				if(auto_adjust_completed_current_index == current_index)
				{
					auto_adjust_completed_cnt++;
				}
				if(auto_adjust_completed_cnt > ((CHG_CURR_AUTO_ADJUST_MAX_INDEX-1)*CHARGING_CHIP_IS_DPM_MODE_COUNT +6))
				{
					dpm_mode_cnt = 0;
					if(current_index == 0)
				...
				if(dpm_mode_cnt >= CHARGING_CHIP_IS_DPM_MODE_COUNT )/*need to decrease charging current*/
				{
					dpm_mode_cnt = 0;
					battery_log(BAT_LOG_CRTI, "decreasing charging current\n");
					g_temp_input_CC_value = CHARGE_CURRENT_3000_00_MA;
					current_index--;
					if(current_index < 0)
						current_index = 0;
					g_temp_CC_value = charging_current_auto_adjust_array[current_index];//CHARGE_CURRENT_1000_00_MA;
					printk("auto_adjust_charging_current =%d,current_index =%d\n",g_temp_CC_value,current_index);
				}
					battery_charging_control(CHARGING_CMD_SET_INPUT_CURRENT,
							 &g_temp_input_CC_value);
					battery_charging_control(CHARGING_CMD_SET_CURRENT, &g_temp_CC_value);
				auto_adjust_completed_current_index = current_index;
			}
			else
			{
				battery_log(BAT_LOG_CRTI, "[chg_cur_auto_adjust]backlight off delay time\n");
				backlight_off_delay_time = 1;
			}
		printk("[chg_cur_auto_adjust]auto_adjust_completed_cnt=%d,auto_adjust_completed_current_index=%d, dpm_mode_cnt=%d,backlight_off_delay_time=%d,auto_adjust_charging_current=%d,g_temp_input_CC_value=%d,current_index=%d\r\n",
		 auto_adjust_completed_cnt,auto_adjust_completed_current_index,dpm_mode_cnt, backlight_off_delay_time, g_temp_CC_value, g_temp_input_CC_value,current_index);
	}
#endif
	else {
#ifdef	MTK_CHARGING_CURRENT_AUTO_ADJUST		
		backlight_off_delay_time = 0;
		charger_first_plug_in_delay = 1;
#endif
		/*HW initialization */
		battery_charging_control(CHARGING_CMD_INIT, NULL);
		...
		#ifndef NEED_COOLER_BCCT
		g_bcct_flag = 0;//2016.6.16 add,do not bcct
		#endif 
