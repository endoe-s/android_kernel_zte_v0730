...
static int g_fg_battery_id = 0;
static int g_bat_id_volt = 0;
...
static ssize_t show_bat_id(struct device *dev, struct device_attribute *attr,
					   char *buf)
{
	bm_print(BM_LOG_CRTI, "[FG] battery id : %d\n", g_fg_battery_id);
	return sprintf(buf, "bat_id:%d,bat_id_volt:%dmV\n", g_fg_battery_id,g_bat_id_volt);
}

static ssize_t store_bat_id(struct device *dev, struct device_attribute *attr,
					    const char *buf, size_t size)
{
	return size;
}

static DEVICE_ATTR(bat_id, 0664, show_bat_id,store_bat_id);
#endif 
#endif
...
{
	printk("[fgauge_get_saddles]sizeof(battery_profile_temperature) / sizeof(BATTERY_PROFILE_STRUCT=%ld\n",sizeof(battery_profile_temperature) / sizeof(BATTERY_PROFILE_STRUCT));
...
int fgauge_get_saddles_r_table(void)
{
	...
	printk("[fgauge_get_saddles_r_table]ssizeof(r_profile_temperature) / sizeof(R_PROFILE_STRUCT=%ld\n",sizeof(r_profile_temperature) / sizeof(R_PROFILE_STRUCT));
...
/* 5. Logging */
	bm_print(BM_LOG_CRTI,
		 "[FGADC] %d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%dmV\r\n",
		 ...
		 batt_meter_cust_data.car_tune_value, batt_meter_cust_data.aging_tuning_value,gFG_15_vlot,g_fg_battery_id,g_bat_id_volt);
#else
		bm_print(BM_LOG_CRTI,
		 "[FGADC] %d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d\r\n",
		 gFG_Is_Charging, gFG_current, gFG_columb, gFG_voltage, gFG_capacity_by_v,
		 gFG_capacity_by_c, gFG_capacity_by_c_init, gFG_BATT_CAPACITY,
		 gFG_BATT_CAPACITY_aging, gFG_compensate_value, gFG_ori_voltage,
		 batt_meter_cust_data.ocv_board_compesate, batt_meter_cust_data.r_fg_board_slope,
		 gFG_voltage_init, batt_meter_cust_data.minerroroffset, gFG_DOD0, gFG_DOD1,
		 batt_meter_cust_data.car_tune_value, batt_meter_cust_data.aging_tuning_value,gFG_15_vlot);
#endif
...
#define V_CURRENT_CHG_ID  (13)  //AUXIN3
#define ILIM_MAX (2377)  //IINMAX = (1000mV/RILIM)*KLIM = (1000/183)*435 (bq24298 DataSheet p24)
#elif defined (CONFIG_MTK_BQ25896_SUPPORT)
#define V_CURRENT_CHG_ID  (13)  //AUXIN3
#define ILIM_MAX (2367)  //IINMAX = (1000mV/RILIM)*KLIM = (1000/150)*355 (bq25896 DataSheet p26)
#endif 
signed int battery_meter_get_charging_current(void)
{
    ...
    int ICharging = 0;
	int ret = -1;
    ret = IMM_GetOneChannelValue_Cali(V_CURRENT_CHG_ID, &v_ilim);
    if ( ret != 0 )
    {
        bm_print(BM_LOG_CRTI, "[mt_battery_GetBatteryData]id_volt read fail\n");
    }
    v_ilim = v_ilim / 1000;//uV to mV, avoid int overflow
    ICharging = (v_ilim * ILIM_MAX) / 1000 ;
	return ICharging;
#elif defined(CONFIG_MTK_BQ25896_SUPPORT)
    int v_ilim = 0;
    int ICharging = 0;
...
#ifdef MTK_MULTI_BAT_PROFILE_SUPPORT
#ifdef MTK_GET_BATTERY_ID_BY_AUXADC
	ret_device_file = device_create_file(&(dev->dev), &dev_attr_bat_id);
#endif 
#endif 
